...
追书小说功能 = 加载功能模块("功能文件.管理功能.小说功能.小说.追书小说")
番茄小说功能 = 加载功能模块("功能文件.管理功能.小说功能.小说.番茄小说")
得间解密功能 = 加载功能模块("功能文件.管理功能.小说功能.功能.得间解密")
得间小说功能 = 加载功能模块("功能文件.管理功能.小说功能.小说.得间小说")
点众小说功能 = 加载功能模块("功能文件.管理功能.小说功能.小说.点众小说")
盐言小说功能 = 加载功能模块("功能文件.管理功能.小说功能.小说.盐言小说")
...
class MyPlugin(Star):
    ...
    async def initialize(self):
        ...
        权限工具.同步管理员白名单(self.config, 强制=True)
        logger.info("馒头bot加载完成：版本=%s, 源码目录=%s", 插件版本, 插件目录)
        try:
            await QQ阅读功能.加载保存的QQ阅读登录态(self.config)
        except Exception as _exc:
            logger.debug(f"加载QQ阅读登录态失败：错误={_exc}")
        await 文件缓存工具.停止每日文件缓存任务(self._文件缓存工具任务)
        self._文件缓存工具任务 = None
        await 小说网盘功能.停止恢复待续传上传任务()
        await asyncio.to_thread(文件缓存工具.初始化文件缓存目录)
        迁移数量 = await asyncio.to_thread(文件缓存工具.迁移旧文件缓存)
        if any(迁移数量.values()):
            logger.info(
                "文件缓存迁移完成：小说=%s，媒体=%s，临时=%s，消息记录=%s，其他=%s",
                迁移数量.get("novel", 0),
                迁移数量.get("media", 0),
                迁移数量.get("temporary", 0),
                迁移数量.get("message", 0),
                迁移数量.get("other", 0),
            )
        临时清理数量 = await asyncio.to_thread(文件缓存工具.清理过期临时文件缓存)
        if 临时清理数量:
            logger.info("清理过期临时文件缓存：数量=%s", 临时清理数量)
        文件缓存工具.重置进程上传占用()
        已清理残留数 = 文件缓存工具.清理残留小说缓存()
        已清理过期数 = 文件缓存工具.清理过期小说缓存()
        if 已清理残留数:
            logger.info(f"插件重载清理遗留小说缓存：数量={已清理残留数}")
        if 已清理过期数:
            logger.info(f"插件加载清理过期小说缓存：数量={已清理过期数}")

        def _记录每日缓存清理(清理数量):
            if 清理数量:
                logger.info("每日零点清理过期小说缓存：数量=%s", 清理数量)

        self._文件缓存工具任务 = 文件缓存工具.启动每日文件缓存任务(
            清理完成回调=_记录每日缓存清理,
        )
        await 小说网盘功能.停止每日网盘远端清理任务()
        小说网盘功能.启动每日网盘远端清理任务(self.config)
        小说网盘功能.启动恢复待续传上传任务(self.config)
        self._帮助网页服务 = await 帮助网页.启动帮助网页服务(self.config)
        ...
        if 回复内容 is None:
            ...
            if 追书回复流 is not None:
                if not await 小说功能开关.异步当前事件可使用小说功能(
                    ...
                    async for 输出内容 in _输出文本回复(
                        小说功能开关.获取小说功能关闭回复("追书", self.config)
                    ):
                        yield 输出内容
                    event.stop_event()
                    return
                小说下载任务.启动小说回复流任务(
                    event, 追书回复流, 帮助功能, 权限工具
                )
                event.stop_event()
                return
            ...
            if 盐言回复流 is not None:
                if not await 小说功能开关.异步当前事件可使用小说功能(event, "盐言", self.config):
                    ...
                    event.stop_event()
                    return
                小说下载任务.启动小说回复流任务(event, 盐言回复流, 帮助功能, 权限工具)
                event.stop_event()
                return
