...
try:
    from 功能文件.API功能.聚合API import 番茄小说 as 聚合API番茄小说
except Exception as 异常:
    聚合API番茄小说 = None
    logger.warning(f'聚合API番茄小说模块加载失败：error={异常}')
try:
    from 功能文件.API功能.Y6api import 番茄小说 as Y6API番茄小说
except Exception as 异常:
    Y6API番茄小说 = None
    logger.warning(f'Y6api番茄小说模块加载失败：error={异常}')
...
def 处理番茄小说API指令(事件: Any, 命令文本: str, 配置: Any) -> str | None:
    ...
    if 文本.lower() == '查看api':
        ...
        return '\n'.join([
            ...
            '4. 聚合API（101.35.133.34:5000，独立接口）',
            '5. Y6api（y68-napi.hf.space，后端打包下载）',
            f'请在 {API选择等待秒数} 秒内发送 1、2、3、4 或 5 完成切换',
        ])
        ...
        async with aiohttp.ClientSession(timeout=超时, headers=浏览器请求头) as 会话:
            ...
            if 接口来源 == 'Y6api':
                ...
                if not 准备结果.get('success'):
                    yield f"番茄小说下载失败：{限制文本长度(准备结果.get('error') or 'Y6api准备失败', 500)}"
                    return
                书籍信息 = 准备结果.get('book_info') or 默认书籍信息(书籍编号)
                章节列表 = 准备结果.get('chapters') or []
                logger.debug(f"番茄小说开始下载：source=Y6api, book_id={书籍编号}, title={书籍信息.get('title')}, author={书籍信息.get('author')}, chapters={len(章节列表)}")
                yield 格式化下载提示(书籍信息, len(章节列表))
                下载结果 = await Y6API番茄小说.下载完整小说(会话, 书籍编号, 书籍信息, 章节列表)
                if not 下载结果.get('success'):
                    yield f"番茄小说下载失败：{限制文本长度(下载结果.get('error') or 'Y6api下载失败', 500)}"
                    return
                ...
                if not 准备结果.get('success'):
                    yield f"番茄小说下载失败：{限制文本长度(准备结果.get('error') or '聚合API准备失败', 500)}"
                    return
                书籍信息 = 准备结果.get('book_info') or 默认书籍信息(书籍编号)
                章节列表 = 准备结果.get('chapters') or []
                logger.debug(f"番茄小说开始下载：source=聚合API, book_id={书籍编号}, title={书籍信息.get('title')}, author={书籍信息.get('author')}, chapters={len(章节列表)}")
                yield 格式化下载提示(书籍信息, len(章节列表))
