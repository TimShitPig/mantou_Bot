...
文件组件缓存删除延迟 = 600
API选择等待秒数 = 120
API选项 = {'1': 'OIAPI', '2': '析API', '3': '崩溃API', '4': '聚合API'}
API直接切换 = {'oiapi': 'OIAPI', '析api': '析API', '崩溃api': '崩溃API', '聚合api': '聚合API'}
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=超时, headers=浏览器请求头) as 会话:
            ...
            if 接口来源 == '聚合API':
                ...
                if not 接口key:
                    yield '番茄小说下载失败：缺少插件配置 番茄小说key；如需使用析API、崩溃API或聚合API，请发送"查看API"后选择 2、3 或 4'
                    return
                if not 书籍编号:
                    yield '没有识别到番茄小说链接'
                    return
