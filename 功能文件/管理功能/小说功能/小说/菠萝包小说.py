...
def 提取菠萝包来源(event: Any, command: Any) -> str | None:
    ...
    for value in values:
        if "sfacg" not in value.lower() and "菠萝包" not in value:
            continue
        match = re.search(r"(?:novel[_-]?id|book[_-]?id|novelid|bookid|novel)\D{0,8}(\d{4,})", value, re.I)
        if match:
        ...
        for key in ("novelid", "novel_id", "bookid", "book_id", "id"):
            for value in query.get(key, []):
                if re.fullmatch(r"\d{4,}", str(value)):
                    return str(value)
        match = re.search(r"(?:novel|book|detail|read)[^0-9]{0,20}(\d{4,})", parsed.path, re.I)
        if match:
            return match.group(1)
    except Exception:
        pass
    match = re.search(r"(?:novel[_-]?id|book[_-]?id|novel|book)\D{0,8}(\d{4,})", text, re.I)
...
def 创建菠萝包HTTP会话(concurrency: int = 菠萝包并发数) -> aiohttp.ClientSession:
    ...
    return aiohttp.ClientSession(headers=菠萝包请求头, timeout=timeout, connector=connector)


async def _请求(session: aiohttp.ClientSession, url: str, *, params: dict[str, Any] | None = None) -> dict[str, Any]:
...
async def 获取菠萝包目录(session: aiohttp.ClientSession, book_id: str) -> list[dict[str, str]]:
    data = await _请求(session, f"{菠萝包API地址}/novels/{urllib.parse.quote(book_id)}/dirs")
    if not _接口成功(data):
        return []
    payload = data.get("data") if isinstance(data.get("data"), dict) else {}
    chapters: list[dict[str, str]] = []
    for volume in payload.get("volumeList") if isinstance(payload.get("volumeList"), list) else []:
        if not isinstance(volume, dict):
            continue
        for chapter in volume.get("chapterList") if isinstance(volume.get("chapterList"), list) else []:
...
async def 获取菠萝包详情(session: aiohttp.ClientSession, book_id: str) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    data = await _请求(session, f"{菠萝包API地址}/novels/{urllib.parse.quote(book_id)}", params={"expand": expand})
    if not _接口成功(data):
        return {}, []
    ...
    status = "完结" if str(status_value).lower() in {"1", "2", "finish", "finished", "完结"} else "连载"
    return {
        "title": title,
...
async def _下载章节(session: aiohttp.ClientSession, chapter: dict[str, str], sem: asyncio.Semaphore) -> str:
    async with sem:
        for attempt in range(菠萝包重试次数):
            try:
                data = await _请求(session, f"{菠萝包API地址}/Chaps/{urllib.parse.quote(chapter['id'])}", params={"expand": "content,needFireMoney,originNeedFireMoney,tsukkomi", "autoOrder": "false"})
                if not _接口成功(data):
                    raise RuntimeError("正文业务失败")
                node = data.get("data") if isinstance(data.get("data"), dict) else {}
                expand = node.get("expand") if isinstance(node.get("expand"), dict) else {}
                content = _文本(expand.get("content") or node.get("content"))
                if content:
                    return content
                raise RuntimeError("正文为空")
            except Exception as exc:
                logger.debug("菠萝包小说章节获取失败：章节=%s, 错误类型=%s", chapter.get("id"), type(exc).__name__)
                if attempt + 1 < 菠萝包重试次数:
                    await asyncio.sleep(0.2 * (attempt + 1))
    return ""


async def 下载菠萝包正文(session: aiohttp.ClientSession, chapters: list[dict[str, str]]) -> list[str]:
    ...
    async def one(index: int, chapter: dict[str, str]) -> None:
        ...
        async with lock:
            ...
            if done >= next_log or done == len(chapters):
                ...
                logger.info("菠萝包小说章节进度：进度=%s/%s, 百分比=%s%%, 成功=%s, 失败=%s, 并发数=%s, 会话复用=开启, 解密方式=无需解密", done, len(chapters), int(done * 100 / max(1, len(chapters))), success, done - success, 菠萝包并发数)
                next_log += max(1, len(chapters) // 10)

    await asyncio.gather(*(one(index, chapter) for index, chapter in enumerate(chapters)))
    return results
...
def _文件名(value: Any) -> str:
    return re.sub(r"[\\/:*?\"<>|\r\n\t]+", "_", str(value or "未知")).strip(" .")[:80] or "菠萝包小说"


def 生成菠萝包文件(book_id: str, detail: dict[str, Any], chapters: list[dict[str, str]], contents: list[str]) -> tuple[str, bytes]:
    status = str(detail.get("status") or "连载")
    title = str(detail.get("title") or f"菠萝包小说{book_id}")
    author = str(detail.get("author") or "未知")
    lines = [菠萝包声明, "", f"名称：{title}", f"作者：{author}", f"状态：{status}", f"字数：{_字数(detail.get('word_count'))}", f"书籍ID：{book_id}", f"章节数：{len(chapters)}", ""]
    if detail.get("intro"):
...
def _缓存(filename: str, content: bytes) -> Path:
    ...
    for index in range(1000):
        path = 菠萝包缓存目录 / (Path(filename).name if index == 0 else f"{Path(filename).stem}_{index}.txt")
        if not path.exists():
...
def _后台(config: Any, path: Any, filename: str) -> None:
    ...
    async def task() -> None:
        try:
            ...
            logger.warning("菠萝包小说百度后台备份异常：错误类型=%s", type(exc).__name__)
        finally:
            小说缓存工具.删除下载缓存文件(path)
...
async def 生成菠萝包下载回复流(event: Any, source: str, config: Any = None) -> AsyncIterator[Any]:
    ...
    try:
        async with 创建菠萝包HTTP会话() as session:
            ...
            if not detail or not chapters:
                yield "下载失败"
                return
            logger.info("菠萝包小说开始下载：书籍编号=%s, 章节数=%s, 并发数=%s, 会话复用=开启, 解密方式=无需解密", book_id, len(chapters), 菠萝包并发数)
        ...
        if any(not content for content in contents):
            logger.warning("菠萝包小说正文不完整：书籍编号=%s, 成功=%s, 总数=%s", book_id, sum(bool(x) for x in contents), len(chapters))
            yield "下载失败"
            return
...
def 获取菠萝包小说回复流(event: Any, command: str, config: Any = None) -> AsyncIterator[Any] | None:
...
async def 搜索小说(keyword: str, *, 需要数量: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        async with 创建菠萝包HTTP会话(4) as session:
            data = await _请求(session, 菠萝包搜索地址, params={"op": "search", "keyword": keyword, "page": 1})
        ...
        for item in rows:
            ...
            if not book_id.isdigit() or not title:
                continue
            result.append({"book_id": book_id, "title": title, "author": _文本(item.get("AuthorName") or "未知"), "url": 构造菠萝包链接(book_id), "intro": _文本(item.get("TypeName")), "score": 0, "heat": 0, "word_count": 0})
