...
酷匠并发数 = 80
酷匠重试次数 = 3
酷匠缓存目录 = 小说缓存工具.下载缓存目录
酷匠声明 = "声明：本文件由机器人自动整理生成，仅供个人学习交流和临时阅读使用。内容版权归原作者及相关平台所有，请勿用于商业用途或二次传播。如喜欢本书，请支持正版。"
酷匠目录请求头 = {
    "auth-code": "dc67efdd82941586e69207b3374037b2",
    "app": "com.dpx.kujiang",
    "platform": "android",
    "device-uuid": "5dd1f054b2f013b9",
    "version": "3.9.7",
    "channel": "XIAOMI",
    "User-Agent": "KuJiang/3.9.7",
    "Accept": "application/json",
}
酷匠正文请求头 = {
    "auth-code": "440590fab1b828085ab67fdc9fc40cbb",
    "app": "com.dpx.kujiang",
    "platform": "android",
    "device-uuid": "A589D18F6E1F84A2",
    "version": "3.9.14",
    "channel": "QQ",
    "User-Agent": "KuJiang/3.9.14(Android;P40;7.1.2)",
    "Accept": "application/json",
}
...
def 提取酷匠来源(event: Any, command: Any) -> str | None:
    ...
    for value in values:
        if "酷匠" not in value and "kujiang" not in value.lower():
            continue
        match = re.search(
            r"(?:book[_-]?id|bookId|novelId|book)\D{0,8}(\d+)", value, re.I
        )
        if match:
    ...
    for attempt in range(酷匠重试次数):
        try:
            async with session.get(
                酷匠基础地址 + path, params=params, headers=headers
            ) as response:
...
def _成功(data: Any) -> bool:
    return (
        isinstance(data, dict) and str((data.get("header") or {}).get("result")) == "0"
    )
...
async def 获取酷匠详情(
    ...
    detail = await _请求(
        session,
        f"/get_book_infos",
        params={"book": book_id, "subsite": "m", "from": "search"},
    )
    if not _成功(detail):
        return {}, []
    info = (
        (detail.get("body") or {}).get("bookinfo")
    ...
    catalog = await _请求(
        ...
        params={
            "book": book_id,
            "auth_code": "dc67efdd82941586e69207b3374037b2",
            "sort": "asc",
        },
        headers=酷匠目录请求头,
    )
    ...
    volumes = (
        (catalog.get("body") or {}).get("catalog")
        if isinstance(catalog.get("body"), dict)
        else []
    )
        ...
        chapters.extend(
            {
                ...
                "title": _文本(chapter.get("v_chapter") or "章节"),
            }
            for chapter in (
                volume.get("chapters")
    ...
    detail_info = {
        "title": _文本(info.get("v_book")),
        "author": _文本(info.get("penname") or "未知"),
        "intro": _文本(info.get("intro")),
        "status": "完结"
        if str(info.get("status") or "").lower() in {"1", "finish", "完结"}
        else "连载",
        "word_count": info.get("public_size") or info.get("word_count") or 0,
    }
...
def _解密正文(value: str) -> str:
    ...
    try:
        ...
        decrypted = AES.new(
            b"KujiangApp747605", AES.MODE_CBC, b"5efd3f6060e20330"
        ).decrypt(encrypted)
        ...
        for attempt in range(酷匠重试次数):
            try:
                data = await _请求(
                    session,
                    "/read",
                    params={"book": book_id, "chapter": chapter["id"]},
                    headers=酷匠正文请求头,
                )
                node = (
                    (data.get("body") or {})
    ...
    async def one(index: int, chapter: dict[str, str]) -> None:
        ...
        async with lock:
            ...
            if done >= next_log or done == len(chapters):
                ...
                logger.info(
                    "酷匠小说章节进度：书籍编号=%s, 进度=%s/%s, 百分比=%s%%, 成功=%s, 失败=%s, 并发数=%s, 会话复用=开启, 解密方式=库",
                    book_id,
                    done,
                    len(chapters),
...
def 生成酷匠文件(
    ...
    title, author = (
        str(detail.get("title") or f"酷匠小说{book_id}"),
            ...
            result.append(
                {
                    "book_id": book_id,
                    "title": title,
                    "author": _文本(item.get("penname") or "未知"),
                    "url": 构造酷匠链接(book_id),
                    "intro": _文本(item.get("intro")),
                    "score": 0,
                    "heat": 0,
                    "word_count": item.get("public_size") or 0,
                }
            )
