...
宜搜链接正则 = re.compile(r"https?://(?:[^\s/<>\"']*\.)?(?:ieasou\.com|easou\.com)[^\s<>\"']*", re.I)
宜搜缓存目录 = Path(__file__).resolve().parents[3] / "下载缓存"
宜搜并发数 = 50
宜搜重试次数 = 3
...
def _文本(value: Any) -> str:
    return re.sub(r"[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]", "", html.unescape(str(value or ""))).strip()
...
def 提取宜搜来源(event: Any, command: Any) -> str | None:
    ...
    for value in values:
        if "宜搜" not in value and "ieasou" not in value.lower() and "easou" not in value.lower():
            continue
        nid = re.search(r"(?:nid|book[_-]?id|bookId|novelId)\D{0,8}(\d{3,})", value, re.I)
...
def 解析宜搜书籍编号(source: str) -> str:
    ...
    match = re.search(r"(?:book|novel|detail)[^0-9]{0,20}(\d{3,})[_/-](\d{1,})", text, re.I)
    if match:
...
def _公共参数(timestamp: str) -> dict[str, str]:
    return {"ac": "999", "appType": "0", "appid": "10001", "appverion": "508500", "bidType": "0", "birt": "1706674841000", "ch": "blf1298_10928_001", "chType": "6", "cid": "eef_easou_book", "dzh": "1", "gender": "1", "instId": timestamp, "instime": timestamp, "os": "android", "pr": "-1.0", "ptype": "5", "pushid": "7b4aaf1210a5bdbac3cea26d5030a419", "recSw": "1", "rtype": "2", "scp": "0", "session_id": "153F4EEE16F56A43FD63ZD21B866413ED9BE044EFB876A115C62DBED82EE4C824D", "showj": "1", "tm": "0", "udid": "3d3ec742930b635fc4c61f0575dbc4d2939edbe2", "userInitPay": "3", "utype": "0", "vm": "5.8.5"}


def _签名地址(path: str, params: dict[str, Any]) -> str:
    items = [(str(key), str(value)) for key, value in params.items() if key != "snk" and value not in (None, "")]
    items.sort()
    sign = hashlib.md5(("&".join(f"{key}={value}" for key, value in items) + "&key=" + 宜搜签名密钥).encode()).hexdigest().upper()
    query = dict(params)
    query["snk"] = sign
...
def 创建宜搜HTTP会话(concurrency: int = 宜搜并发数) -> aiohttp.ClientSession:
    ...
    return aiohttp.ClientSession(headers={"User-Agent": "esbook android 5.8.5", "Accept": "application/json"}, timeout=timeout, connector=connector)
...
def _成功(data: Any) -> bool:
    return isinstance(data, dict) and ("success" not in data or data.get("success") in (True, 1, "1", "true"))
...
def _取章节(data: dict[str, Any]) -> list[dict[str, str]]:
    ...
    pools.extend(item.get("chapters") for item in data.get("volumes", []) if isinstance(item, dict))
    seen: set[str] = set()
    for pool in pools:
        ...
        for item in pool:
            ...
            if not number.isdigit() or number in seen:
                continue
            seen.add(number)
            rows.append({"id": number, "title": _文本(item.get("chapter_name") or item.get("name") or f"第{number}章")})
    rows.sort(key=lambda item: int(item["id"]))
    return rows


async def 获取宜搜详情(session: aiohttp.ClientSession, book_id: str) -> tuple[dict[str, Any], list[dict[str, str]]]:
    parts = _编号(book_id)
    if not parts:
        return {}, []
    nid, gid = parts
    detail_params = _公共参数(_时间参数())
    detail_params.update({"ad": "0", "gid": gid, "nid": nid, "sort": "1", "size": "50", "returnType": "010", "gsort": "1"})
    detail_data = await _请求(session, _签名地址("/api/bookapp/bookSummary.m", detail_params))
    if not _成功(detail_data):
        return {}, []
    info = detail_data.get("coverInfo") if isinstance(detail_data.get("coverInfo"), dict) else {}
    ...
    while offset <= 200000:
        ...
        page_params.update({"ad": "0", "gid": gid, "nid": nid, "sort": str(offset), "size": str(page_size), "returnType": "100", "gsort": "1"})
        data = await _请求(session, _签名地址("/api/bookapp/bookSummary.m", page_params))
        if not _成功(data):
            break
        ...
        for chapter in page_chapters:
            ...
            if chapter_id and chapter_id not in seen:
                ...
                new_count += 1
        if bool(data.get("lastPage")) or len(page_chapters) < page_size or new_count == 0:
            break
    ...
    status = "完结" if status_value in {"1", "2", "finish", "finished", "完本", "完结"} else "连载"
    return ({"title": _文本(info.get("name")), "author": _文本(info.get("author") or "未知"), "intro": _文本(info.get("desc")), "status": status, "word_count": info.get("wordCount") or info.get("words") or 0}, chapters)
...
async def _下载章节(session: aiohttp.ClientSession, book_id: str, chapter: dict[str, str], sem: asyncio.Semaphore) -> str:
    ...
    async with sem:
        for attempt in range(宜搜重试次数):
            try:
                params = _公共参数(_时间参数())
                params.update({"a": "1", "autoBuy": "0", "gid": gid, "nid": nid, "sort": chapter["id"], "gsort": "0", "sgsort": "0", "sequence": "1"})
                data = await _请求(session, _签名地址("/api/bookapp/chargeChapter.m", params))
                ...
                plain = unpad(DES.new(宜搜解密密钥, DES.MODE_CBC, 宜搜解密向量).decrypt(bytes.fromhex(value)), DES.block_size).decode("utf-8", "replace")
                plain = _文本(plain).strip()
                if plain:
                    return plain
            except Exception as exc:
                logger.debug("宜搜小说章节获取失败：章节=%s, 错误类型=%s", chapter.get("id"), type(exc).__name__)
                if attempt + 1 < 宜搜重试次数:
                    await asyncio.sleep(0.2 * (attempt + 1))
    return ""


async def 下载宜搜正文(session: aiohttp.ClientSession, book_id: str, chapters: list[dict[str, str]]) -> list[str]:
    ...
    async def one(index: int, chapter: dict[str, str]) -> None:
        ...
        async with lock:
            ...
            if done >= next_log or done == len(chapters):
                logger.info("宜搜小说章节进度：书籍编号=%s, 进度=%s/%s, 百分比=%s%%, 成功=%s, 失败=%s, 并发数=%s, 会话复用=开启, 解密方式=库", book_id, done, len(chapters), int(done * 100 / max(1, len(chapters))), sum(bool(x) for x in results), done - sum(bool(x) for x in results), sem._value)
                next_log += max(1, len(chapters) // 10)
...
def _文件名(value: Any) -> str:
    return re.sub(r"[\\/:*?\"<>|\r\n\t]+", "_", str(value or "未知")).strip(" .")[:80] or "宜搜小说"


def 生成宜搜文件(book_id: str, detail: dict[str, Any], chapters: list[dict[str, str]], contents: list[str]) -> tuple[str, bytes]:
    status = str(detail.get("status") or "连载")
    title = str(detail.get("title") or f"宜搜小说{book_id}")
    author = str(detail.get("author") or "未知")
    lines = [宜搜声明, "", f"名称：{title}", f"作者：{author}", f"状态：{status}", f"字数：{_字数(detail.get('word_count'))}", f"书籍ID：{book_id}", f"章节数：{len(chapters)}", ""]
    if detail.get("intro"):
...
async def 生成宜搜下载回复流(event: Any, source: str, config: Any = None) -> AsyncIterator[Any]:
    ...
    try:
        async with 创建宜搜HTTP会话() as session:
            ...
            if not detail or not chapters:
                yield "下载失败"
                return
            logger.info("宜搜小说开始下载：书籍编号=%s, 章节数=%s, 并发数=%s, 会话复用=开启, 解密方式=库", book_id, len(chapters), 宜搜并发数)
        ...
        if len(contents) != len(chapters) or any(not content for content in contents):
            logger.warning("宜搜小说正文不完整：书籍编号=%s, 成功=%s, 总数=%s", book_id, sum(bool(x) for x in contents), len(chapters))
            yield "下载失败"
            return
...
def 获取宜搜小说回复流(event: Any, command: str, config: Any = None) -> AsyncIterator[Any] | None:
...
async def 搜索小说(keyword: str, *, 需要数量: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        params = _公共参数(_时间参数())
        params.update({"word": keyword, "type": "0", "page_id": "1", "count": str(min(100, max(1, int(需要数量 or 20)))), "sort_type": "0", "subclass": "0", "datasource": "0", "catalog": "0", "bookStatus": "0"})
        async with 创建宜搜HTTP会话(4) as session:
            data = await _请求(session, _签名地址("/api/bookapp/searchdzh.m", params))
        rows = data.get("all_book_items") if isinstance(data.get("all_book_items"), list) else []
        result = []
        for item in rows:
            if not isinstance(item, dict):
                continue
            ...
            if not nid.isdigit() or not gid.isdigit() or not title:
                continue
            book_id = f"{nid}_{gid}"
            result.append({"book_id": book_id, "title": title, "author": _文本(item.get("author") or "未知"), "url": 构造宜搜链接(book_id), "intro": _文本(item.get("desc")), "score": item.get("score") or 0, "heat": item.get("readCount") or 0, "word_count": item.get("wordCount") or item.get("words") or 0})
