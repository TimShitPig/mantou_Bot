...
from typing import Any, AsyncIterator, List, Mapping, Sequence
...
def _make_sbox() -> List[int]:
    ...
    for x in range(256):
        ...
        box.append(
            (inv ^ _rot8(inv, 1) ^ _rot8(inv, 2) ^ _rot8(inv, 3) ^ _rot8(inv, 4) ^ 0x63)
            & 0xFF
        )
    return box
...
def aes128_ecb_pkcs7_encrypt(data: bytes, key: bytes) -> bytes:
    ...
    return b"".join(
        aes128_encrypt_block(data[i : i + 16], key) for i in range(0, len(data), 16)
    )
...
def des_ecb_pkcs7_encrypt(data: bytes, key: bytes) -> bytes:
    ...
    return b"".join(
        des_encrypt_block(data[i : i + 8], key) for i in range(0, len(data), 8)
    )


def selftest_crypto() -> bool:
    aes = aes128_encrypt_block(
        bytes.fromhex("6bc1bee22e409f96e93d7e117393172a"),
        bytes.fromhex("2b7e151628aed2a6abf7158809cf4f3c"),
    ).hex()
    des = des_encrypt_block(
        bytes.fromhex("0123456789abcdef"), bytes.fromhex("133457799bbcdff1")
    ).hex()
...
def _构造Bearer(sessionid: str = "", ts: int | None = None) -> str:
    ...
    signing = (
        _base64url(_紧凑JSON(header)) + "." + _base64url(_紧凑JSON(claims))
    ).encode("ascii")
...
def _提取列表(data: Mapping[str, Any], keys: tuple[str, ...]) -> list[Any]:
    for key in keys:
        ...
        if isinstance(value, Mapping):
            nested = _提取列表(
                value,
                ("books", "bookList", "chapters", "chapterList", "list", "records"),
            )
            if nested:
                return nested
    return []
...
def 解析塔读搜索书籍(response: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in rows:
        if not isinstance(item, Mapping):
            continue
        book = (
            item.get("bookInfo") if isinstance(item.get("bookInfo"), Mapping) else item
        )
        book_id = str(
            book.get("bookId") or book.get("id") or book.get("docId") or ""
        ).strip()
        if not book_id:
            continue
        title = str(
            book.get("bookName") or book.get("title") or book.get("name") or "未知"
        ).strip()
...
def 解析塔读批量章节(response: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(rows, start=1):
        if not isinstance(item, Mapping):
            continue
        chapter_id = str(
            item.get("chapterId") or item.get("chapter_id") or item.get("id") or ""
        ).strip()
        chapter_num = (
            item.get("chapterNum")
        ...
        url = _绝对地址(
            ...
            str(
                ...
                or item.get("chapterUrl")
                or item.get("chapterDownloadUrl")
                or item.get("url")
                or ""
            ),
        )
...
def _塔读请求头(
    sign_params: Mapping[str, Any] | None = None, content_type: str = ""
) -> dict[str, str]:
    state = _塔读会话
    headers = {
...
async def _请求塔读接口(
    ...
    *,
    ...
    url = (
        ...
        else 塔读API.rstrip("/") + "/" + path.lstrip("/")
    )
    headers = _塔读请求头(
        sign_params, "application/x-www-form-urlencoded" if method == "POST" else ""
    )
...
def _更新塔读会话(data: Mapping[str, Any]) -> None:
    ...
    _塔读会话.sessionid = str(
        data.get("sessionId") or data.get("sessionid") or _塔读会话.sessionid or ""
    )
    _塔读会话.token = str(data.get("token") or _塔读会话.token or "")
    _塔读会话.refresh_token = str(
        data.get("refreshToken")
        or data.get("refresh_token")
        or _塔读会话.refresh_token
        or ""
    )
...
async def 确保塔读会话(session: aiohttp.ClientSession) -> None:
    ...
    async with _塔读会话锁:
        ...
        try:
            ...
            logger.debug(
                "塔读游客Token续期失败：阶段=refresh, 错误类型=%s", type(exc).__name__
            )
        await _注册塔读会话(session)
...
async def _获取详情(session: aiohttp.ClientSession, book_id: str) -> dict[str, Any]:
    response = await _请求塔读接口(
        session, "GET", "/book/info/titlePage", params={"bookId": book_id}
    )
    data = _数据对象(response)
...
async def _获取单章地址(
    session: aiohttp.ClientSession, book_id: str, chapter: Mapping[str, Any]
) -> str:
    response = await _请求塔读接口(
        session,
        "GET",
    ...
    chapter_info = (
        ...
        if isinstance(data.get("chapterInfo"), Mapping)
        else data
    )
    return _绝对地址(
        str(chapter_info.get("domain") or data.get("domain") or ""),
        str(
            chapter_info.get("chapterUrl")
            or chapter_info.get("chapterDownloadUrl")
...
async def _下载全部章节(
    ...
    await asyncio.gather(
        *(download_one(index, chapter) for index, chapter in enumerate(chapters))
    )
    return [
        item
        or {
...
def 解析塔读书籍详情(detail: Mapping[str, Any] | None) -> dict[str, str]:
    ...
    author = str(
        _详情字段(data, "bookAuthor", "authorName", "author", "writer") or "未知"
    )
...
async def _准备发送文本文件(
    event: Any, file_name: str, content: bytes, config: Any, *, title: str, author: str
) -> dict[str, Any]:
    ...
    try:
        ...
        sent = await 小说网盘.发送小说下载完成链接(
            event, title, author, str(upload.get("share_url") or "")
        )
        if sent.get("sent"):
            return {
                "sent": True,
        ...
        logger.warning(
            "塔读小说网盘处理失败：文件=%s, 错误类型=%s", file_name, type(exc).__name__
        )
        _删除缓存(cache_path)
        return {
            "sent": False,
...
async def 生成塔读下载回复流(
    ...
    try:
        ...
        send_result = await _准备发送文本文件(
            event, file_name, content, 配置, title=title, author=author
        )
        if send_result.get("sent"):
            _启动百度后台上传并清理(
                配置, send_result.get("source_cache_path"), file_name
            )
            return
...
async def 搜索小说(关键词: str, *, 需要数量: int = 20) -> list[dict[str, Any]]:
    try:
        ...
        logger.warning(
            "塔读小说搜索失败：关键词=%s, 错误类型=%s", 关键词, type(exc).__name__
        )
        return []
