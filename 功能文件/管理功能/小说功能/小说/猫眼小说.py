...
猫眼链接正则 = re.compile(
    r"https?://(?:[^\s/<>\"']*\.)?(?:zhangyue\.com|ireader\.com)[^\s<>\"']*", re.I
)
猫眼并发数 = 80
猫眼重试次数 = 3
猫眼缓存目录 = 小说缓存工具.下载缓存目录
...
def 提取猫眼来源(event: Any, command: Any) -> str | None:
    ...
    for value in values:
        if (
            "猫眼" not in value
            and "zhangyue" not in value.lower()
            and "ireader" not in value.lower()
        ):
            continue
...
def 创建猫眼HTTP会话(concurrency: int = 猫眼并发数) -> aiohttp.ClientSession:
    ...
    return aiohttp.ClientSession(
        headers={
            "User-Agent": "Mozilla/5.0 (Linux; Android 10; Mobile) AppleWebKit/537.36 MaoyanReader/2.4.0",
    ...
    for attempt in range(猫眼重试次数):
        try:
            async with session.get(
                url, params=merged, headers={"Accept": accept}
            ) as response:
...
async def 获取猫眼详情(
    ...
    _, detail = await _请求(
        session, 猫眼详情地址, params={"bid": book_id, "source": "read"}
    )
    ...
    result = {
        ...
        "word_count": info.get("wordCount") or 0,
    }
...
def _正文文本(value: Any) -> str:
    ...
    text = re.sub(
        r"<\/?(?:br|p|div|h[1-6]|li|section|article|blockquote|tr|td)[^>]*>",
        "\n",
        text,
        flags=re.I,
    )
    ...
    async with sem:
        for attempt in range(猫眼重试次数):
            try:
                _, data = await _请求(
                    session,
                    f"{猫眼正文地址}/{urllib.parse.quote(book_id)}/{urllib.parse.quote(chapter['id'])}",
                )
                if not isinstance(data, dict) or str(data.get("code", "0")) not in {
                    "0",
                    "200",
                }:
    ...
    async def one(index: int, chapter: dict[str, str]) -> None:
        ...
        async with lock:
            ...
            if done >= next_log or done == len(chapters):
                ...
                logger.info(
                    "猫眼小说章节进度：书籍编号=%s, 进度=%s/%s, 百分比=%s%%, 成功=%s, 失败=%s, 并发数=%s, 会话复用=开启, 解密方式=无需解密",
                    book_id,
                    done,
                    len(chapters),
...
def 生成猫眼文件(
    ...
    title, author = (
        str(detail.get("title") or f"猫眼小说{book_id}"),
    ...
    try:
        async with 创建猫眼HTTP会话() as session:
            ...
            if not detail or not chapters:
                ...
                return
            logger.info(
                "猫眼小说开始下载：书籍编号=%s, 章节数=%s, 并发数=%s, 会话复用=开启, 解密方式=无需解密",
                book_id,
                len(chapters),
                猫眼并发数,
            )
...
async def 搜索小说(keyword: str, *, 需要数量: int = 20) -> list[dict[str, Any]]:
    ...
    try:
        params = {
            ...
            "keyWord": keyword,
            "type": "book",
            "pageSize": min(50, max(1, int(需要数量 or 20))),
            "currentPage": 1,
            "filterAuthor": 1,
            "suggestType": 0,
        }
        ...
        rows = (
            (((data or {}).get("body") or {}).get("book") or {}).get("datas")
        ...
        for item in rows if isinstance(rows, list) else []:
            if not isinstance(item, dict):
                continue
            info = (
                item.get("data_info") if isinstance(item.get("data_info"), dict) else {}
            )
            book_id, title = (
                str(info.get("bookId") or ""),
                _文本(info.get("displayBookName") or info.get("bookName")),
            )
            ...
            result.append(
                {
                    ...
                    "author": _文本(info.get("bookAuthor") or "未知"),
                    "url": 构造猫眼链接(book_id),
                    "intro": _文本(info.get("bookDescription")),
                    "score": 0,
                    "heat": 0,
                }
            )
