...
规范章节列表键 = {_规范键(项目) for 项目 in 章节列表键}
...
def 解析知乎章节详情(原始: bytes | dict[str, Any], 业务编号: str = "", 章节编号: str = "") -> dict[str, Any]:
    ...
    if 业务编号 and 实际业务编号 and 实际业务编号 != str(业务编号):
        raise RuntimeError("知乎章节详情专栏不匹配")
    if 章节编号 and 实际章节编号 and 实际章节编号 != str(章节编号):
        raise RuntimeError("知乎章节详情章节不匹配")
    ...
    for 尝试次数 in range(3):
        try:
            async with session.get(
                ...
                return 解析知乎章节详情(原始, 业务编号, 章节编号)
        except Exception as exc:
            最后异常 = exc
            if 尝试次数 + 1 < 3:
                await asyncio.sleep(0.2 * (尝试次数 + 1))
    raise RuntimeError("知乎章节详情请求失败") from 最后异常
        ...
        try:
            async with session.get(
                ...
                if not isinstance(数据, dict) or not isinstance(数据.get("data"), list):
                    raise RuntimeError("知乎目录响应格式错误")
                return 数据
        except Exception as exc:
            最后异常 = exc
            if 尝试次数 + 1 < 3:
                await asyncio.sleep(0.25 * (尝试次数 + 1))
    raise _知乎目录接口异常("知乎目录请求失败") from 最后异常
...
async def _获取知乎目录接口(
    ...
    while True:
        ...
        if 页数 > (目录最大章节数 // 目录页大小) + 2:
            raise RuntimeError("知乎目录分页数量异常")
        数据 = await _请求知乎目录页(session, 业务编号, 偏移量)
...
async def _获取知乎目录头部链(
    ...
    async def 加载(章节编号: str) -> dict[str, Any]:
        ...
        if not 章节编号:
            raise RuntimeError("知乎目录章节编号为空")
        if 章节编号 not in 节点:
            if len(节点) >= 目录最大章节数:
                raise RuntimeError("知乎目录章节数量异常")
            节点[章节编号] = await _请求知乎章节详情(session, 业务编号, 章节编号)
        return 节点[章节编号]
    ...
    return {
        ...
        "title": str(首节点.get("column_title") or 首节点.get("title") or "知乎专栏"),
...
def _提取章节(根: Any) -> list[dict[str, str]]:
    结果: list[dict[str, str]] = []
    for 当前, _深度 in _遍历对象(根):
        if not isinstance(当前, dict):
            continue
        for 键, 值 in 当前.items():
            if _规范键(键) not in 规范章节列表键 or not isinstance(值, (list, tuple)):
                continue
            for 项目 in 值:
                if not isinstance(项目, dict):
                    continue
                正文值 = _字段值(项目, 正文键, 最长=True)
                正文 = _内容值转文本(正文值)
                if len(正文) < 2:
                    continue
                标题 = _内容值转文本(_字段值(项目, 标题键)) or f"第{len(结果) + 1}节"
                结果.append({"title": 标题, "content": 正文})
            if 结果:
                return 结果
    return 结果
...
def 解析知乎恢复结果(原始: bytes, 问题编号: str = "", 回答编号: str = "") -> dict[str, Any]:
    ...
    if not 标题 and isinstance(根对象, str):
        标题匹配 = re.search(r"<title[^>]*>([\s\S]*?)</title>", 根对象, re.I)
        if 标题匹配:
            标题 = 清理正文(标题匹配.group(1))
    标题 = 标题 or 章节[0]["title"]
    作者值 = _字段值(根对象, 作者键)
    if isinstance(作者值, dict):
        作者值 = _字段值(作者值, {"name", "nickname", "displayname", "昵称", "姓名"})
    作者 = 清理正文(作者值) or "未知"
    简介 = 清理正文(_字段值(根对象, 简介键))
    字数值 = _字段值(根对象, 字数键, 最长=True)
    字数 = 解析字数(字数值)
    if 字数 <= 0:
...
def 解析知乎章节正文(原始: bytes, 默认标题: str = "") -> dict[str, str]:
    根对象 = _解析响应对象(原始)
    if isinstance(根对象, dict):
        代码 = 根对象.get("code")
        if 代码 not in (None, 0, "0"):
            raise RuntimeError("知乎正文接口返回错误")
    正文值 = _字段值(根对象, 正文键, 最长=True)
    正文 = _内容值转文本(正文值)
...
async def _获取本地令牌(udid: str, *, 刷新: bool = False) -> str:
    async with _令牌锁:
        if not 刷新 and _令牌缓存.get(udid):
            return _令牌缓存[udid]
        token = await asyncio.to_thread(恢复客户端.fetch_guest_token, udid)
        _令牌缓存[udid] = token
        return token


async def _本地恢复(来源: str) -> bytes:
    _明文, 请求体 = await asyncio.to_thread(恢复客户端.calculate_body, 来源)
    for udid in 恢复客户端.DEFAULT_UDIDS:
        try:
            token = await _获取本地令牌(udid)
            for 尝试次数 in range(2):
                状态码, 原始 = await asyncio.to_thread(
                    恢复客户端.restore, 来源, udid, token, 请求体
                )
                if 200 <= 状态码 < 300:
                    return 原始
                if 状态码 in {401, 403} and 尝试次数 == 0:
                    token = await _获取本地令牌(udid, 刷新=True)
                    continue
                break
        except Exception as exc:
...
def _构造知乎章节来源(来源: str, 业务编号: str, 章节编号: str) -> str:
    try:
        原地址 = urlsplit(来源)
        查询参数 = 原地址.query
    except ValueError:
...
async def 下载知乎全部章节(
    ...
    if 待重试:
        await 记录进度(正文重试次数)
        raise RuntimeError(f"知乎章节正文不完整：missing={len(待重试)}")
    await 记录进度(正文重试次数)
...
def _创建知乎会话() -> aiohttp.ClientSession:
    ...
    connector = aiohttp.TCPConnector(
        limit=max(8, 知乎正文最大并发数),
        limit_per_host=max(8, 知乎正文最大并发数),
        ttl_dns_cache=300,
    )
...
def 生成小说文件内容(书籍: dict[str, Any]) -> tuple[str, bytes]:
    书名 = str(书籍.get("title") or "知乎文章").strip()
...
def 生成小说文件名(书籍: dict[str, Any]) -> str:
    ...
    return f"[{状态}]书名：{清理文件名(书籍.get('title') or '知乎文章')} 作者：{清理文件名(书籍.get('author') or '未知')}.txt"
...
def 写入缓存(文件名: str, 文件内容: bytes) -> Path:
    ...
    基础名 = Path(清理文件名(文件名)).name or "知乎文章.txt"
...
def 清理文件名(值: Any) -> str:
    ...
    return 文本[:80] or "知乎文章"


def 解析知乎编号(来源: str) -> tuple[str, str]:
    try:
...
def _是知乎链接(值: str) -> bool:
    try:
        地址 = urlsplit(值)
    except ValueError:
        return False
    主机 = (地址.hostname or "").lower().strip(".")
    return 主机 in 允许主机 and bool(解析知乎编号(值)[0])


def 提取知乎链接(值: Any) -> str:
    if 值 is None:
        return ""
    if isinstance(值, dict):
        for 子值 in 值.values():
...
def 提取直接知乎来源(命令文本: Any) -> str | None:
    链接 = 提取知乎链接(命令文本)
    return 链接 or None


def 提取事件知乎来源(event: Any) -> str | None:
