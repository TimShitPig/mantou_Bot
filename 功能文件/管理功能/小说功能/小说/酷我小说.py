...
酷我链接正则 = re.compile(r"https?://(?:[^\s/<>\"']*\.)?(?:kuwo\.cn|kuwo\.com)[^\s<>\"']*", re.I)
酷我并发数 = 80
酷我重试次数 = 3
...
酷我请求头 = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/115 Safari/537.36", "Accept": "application/json"}
...
def 创建酷我HTTP会话(concurrency: int = 酷我并发数) -> aiohttp.ClientSession:
    ...
    return aiohttp.ClientSession(headers=酷我请求头, timeout=timeout, connector=connector)


async def _请求(session: aiohttp.ClientSession, path: str, *, params: dict[str, Any] | None = None) -> dict[str, Any]:
...
async def 获取酷我详情(session: aiohttp.ClientSession, book_id: str) -> tuple[dict[str, Any], list[dict[str, str]]]:
    detail = await _请求(session, f"/book/{urllib.parse.quote(book_id)}")
    if not _成功(detail):
        return {}, []
    info = detail.get("data") if isinstance(detail.get("data"), dict) else {}
    catalog = await _请求(session, f"/book/{urllib.parse.quote(book_id)}/chapters", params={"paging": 0})
    rows = catalog.get("data") if isinstance(catalog.get("data"), list) else []
    chapters = [{"id": str(item.get("chapter_id") or ""), "title": _文本(item.get("chapter_title") or "章节")} for item in rows if isinstance(item, dict) and str(item.get("chapter_id") or "").strip()]
    detail_info = {"title": _文本(info.get("title")), "author": _文本(info.get("author_name") or "未知"), "intro": _文本(info.get("intro")), "status": "完结" if str(info.get("status") or "").lower() in {"1", "finish", "完结"} else "连载", "word_count": info.get("all_words") or info.get("word_count") or 0}
...
async def _下载章节(session: aiohttp.ClientSession, book_id: str, chapter: dict[str, str], sem: asyncio.Semaphore) -> str:
    async with sem:
        for attempt in range(酷我重试次数):
            try:
                data = await _请求(session, f"/book/{urllib.parse.quote(book_id)}/chapters/{urllib.parse.quote(chapter['id'])}")
                if not _成功(data):
                ...
                logger.debug("酷我小说章节获取失败：章节=%s, 错误类型=%s", chapter.get("id"), type(exc).__name__)
                if attempt + 1 < 酷我重试次数:
                    await asyncio.sleep(0.2 * (attempt + 1))
    return ""


async def 下载酷我正文(session: aiohttp.ClientSession, book_id: str, chapters: list[dict[str, str]]) -> list[str]:
    ...
    async def one(index: int, chapter: dict[str, str]) -> None:
        ...
        async with lock:
            ...
            if done >= next_log or done == len(chapters):
                ...
                logger.info("酷我小说章节进度：书籍编号=%s, 进度=%s/%s, 百分比=%s%%, 成功=%s, 失败=%s, 并发数=%s, 会话复用=开启, 解密方式=无需解密", book_id, done, len(chapters), int(done * 100 / max(1, len(chapters))), success, done - success, 酷我并发数)
                next_log += max(1, len(chapters) // 10)
...
def _文件名(value: Any) -> str:
    return re.sub(r"[\\/:*?\"<>|\r\n\t]+", "_", str(value or "未知")).strip(" .")[:80] or "酷我小说"


def 生成酷我文件(book_id: str, detail: dict[str, Any], chapters: list[dict[str, str]], contents: list[str]) -> tuple[str, bytes]:
    status = str(detail.get("status") or "连载")
    title, author = str(detail.get("title") or f"酷我小说{book_id}"), str(detail.get("author") or "未知")
    lines = [酷我声明, "", f"名称：{title}", f"作者：{author}", f"状态：{status}", f"字数：{_字数(detail.get('word_count'))}", f"书籍ID：{book_id}", f"章节数：{len(chapters)}", ""]
    if detail.get("intro"):
...
def _缓存(filename: str, content: bytes) -> Path:
    ...
    for index in range(1000):
        path = 酷我缓存目录 / (Path(filename).name if index == 0 else f"{Path(filename).stem}_{index}.txt")
        if not path.exists():
...
async def 生成酷我下载回复流(event: Any, source: str, config: Any = None) -> AsyncIterator[Any]:
    ...
    try:
        async with 创建酷我HTTP会话() as session:
            ...
            if not detail or not chapters:
                yield "下载失败"
                return
            logger.info("酷我小说开始下载：书籍编号=%s, 章节数=%s, 并发数=%s, 会话复用=开启, 解密方式=无需解密", book_id, len(chapters), 酷我并发数)
        ...
        if any(not content for content in contents):
            logger.warning("酷我小说正文不完整：书籍编号=%s, 成功=%s, 总数=%s", book_id, sum(bool(x) for x in contents), len(chapters))
            yield "下载失败"
            return
...
def 获取酷我小说回复流(event: Any, command: str, config: Any = None) -> AsyncIterator[Any] | None:
    ...
    try:
        ...
        for item in rows:
            ...
            if not book_id or not title:
                continue
            result.append({"book_id": book_id, "title": title, "author": _文本(item.get("author_name") or "未知"), "url": 构造酷我链接(book_id), "intro": _文本(item.get("intro")), "score": 0, "heat": 0, "word_count": item.get("all_words") or 0})
