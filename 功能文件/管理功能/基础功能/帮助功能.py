        ...
        "小类": [
            {
                ...
                "触发项": [
                    ...
                    {"名称": "QQ阅读", "触发": "QQ阅读详情/目录链接 / QQ阅读登录 / 直接发送QQ阅读Cookie / QQ阅读cookie状态", "详情": "识别 QQ阅读链接，详情、目录和正文统一走本地 App 签名接口并使用动态密钥池解密；完成后上传 UC 网盘并发送“点击打开”按钮。群文件清理管理员白名单 QQ 可手机号登录或直接发送浏览器 Cookie、curl、Cookie Editor JSON、cookies.txt 保存登录态到数据库，状态不展示 Cookie 原文。"},
