        ...
        "小类": [
            {
                ...
                "触发项": [
                    ...
                    {"名称": "全量消息", "触发": "全量消息 / 全量消息状态", "详情": "只能查询 QQ 官方群消息接收状态，不能通过机器人直接开启或关闭；需要在 QQ 机器人资料页的群消息接收设置中手动选择群内全部消息或其他范围。官方接口不可用时不伪造状态。"},
                    {"名称": "小说开关", "触发": "小说 / 小说列表 / 开番茄 / 开七猫 / 开书旗 / 开QQ阅读 / 开得间 / 开点众 / 开知乎 / 开塔读 / 关闭对应功能 / 开测试 / 关测试", "详情": "发送「小说」或「小说列表」可查看全部小说功能的开启/关闭状态；管理员可用「开平台名」或「开启平台名」开启番茄、七猫、书旗、QQ阅读、得间、点众、知乎和塔读下载功能。开启管理员测试模式后，管理员可继续测试已关闭的小说平台，普通用户仍按平台开关处理。", "快捷命令": ["小说", "开番茄", "开七猫", "开书旗", "开QQ阅读", "开得间", "开点众", "开知乎", "开塔读", "开测试", "关测试"]},
        ...
        "小类": [
            {
                "名称": "小说功能",
                "触发项": [
                    {"名称": "找书", "触发": "找关键词 / 找书 关键词 / 找书名 关键词 / 找作者 关键词 / 上一页 / 下一页", "详情": "聚合搜索番茄、七猫、书旗、得间、点众；可自动识别或显式指定书名、作者，按匹配方式、实际匹配字数、覆盖率和跨平台共识排序，无关结果不展示；每页 5 条，不显示链接和来源；同书优先番茄>七猫>书旗>得间>点众。"},
            ...
            },
            {
                ...
                "触发项": [
                    ...
                    {"名称": "数字撤回", "触发": "连续 9-12 位数字", "详情": "QQ群主和管理员不撤回；普通成员撤回当前消息并拉取 100 条群历史，最多撤回该用户最近 8 条消息，成功后按群 5 分钟限频 @配置管理员和发送者。"},
                    {"名称": "卡片撤回", "触发": "群名片/JSON 卡片/群分享卡片", "详情": "QQ群主和管理员不撤回；普通成员自动撤回当前卡片消息，并最多撤回该用户最近 8 条消息，成功后按群 5 分钟限频 @配置管理员和发送者，通知不展示数字和链接。"},
                    {"名称": "合并转发撤回", "触发": "合并转发/聊天记录", "详情": "QQ群主和管理员不撤回；普通成员自动撤回当前合并转发消息，并最多撤回该用户最近 8 条消息，成功后按群 5 分钟限频 @配置管理员和发送者。"},
                    {"名称": "QQ闪传撤回", "触发": "QQ闪传", "详情": "QQ群主和管理员不撤回；普通成员自动撤回当前 QQ 闪传消息，并最多撤回该用户最近 8 条消息，成功后按群 5 分钟限频 @配置管理员和发送者。"},
                ],
            },
        ],
    },
