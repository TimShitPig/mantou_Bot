        ...
        "小类": [
            {
                ...
                "触发项": [
                    ...
                    {"名称": "番茄小说API切换", "触发": "查看API -> 1 / 2 / 3 或直接发送 oiapi / 析api / 崩溃api", "详情": "发送 查看API 后在 120 秒内发送 1/2/3 切换，或直接发送 oiapi/析api/崩溃api 快速切换。", "快捷命令": ["oiapi", "析api", "崩溃api"]},
                ],
            },
            {
                "名称": "管理功能",
                "触发项": [
                    ...
                    {"名称": "群文件清理", "触发": "清理群文件 / 群文件清理 / 清理全部群文件", "详情": "扫描并发删除当前群或机器人所在全部群的群文件。", "快捷命令": ["清理群文件", "清理全部群文件"]},
                    {"名称": "授权链接", "触发": "授权 / 授权 数字群号 / 授权 数字群号 机器人QQ", "详情": "生成 QQ 群服务授权链接。"},
                    {"名称": "小说开关", "触发": "开启番茄 / 关闭番茄 / 开启七猫 / 关闭七猫", "详情": "管理员可开关番茄小说和七猫小说下载功能。", "快捷命令": ["开启番茄", "关闭番茄", "开启七猫", "关闭七猫"]},
                    {"名称": "付费开关", "触发": "开启收费 / 关闭收费 / 开启群聊收费 / 关闭群聊收费 / 开启私聊收费 / 关闭私聊收费", "详情": "关闭收费让未单独设置开关的范围免费，已单独开启收费的群聊/私聊仍收费；开启收费强制全部收费。", "快捷命令": ["开启收费", "关闭收费", "开启群聊收费", "关闭群聊收费", "开启私聊收费", "关闭私聊收费"]},
...
def 进入大类触发项(会话键: str, 编号文本: str) -> str:
    编号 = int(编号文本)
    if 编号 < 1 or 编号 > len(帮助大类):
        return f"帮助编号无效，请发送 1-{len(帮助大类)}"
    设置帮助状态(会话键, "触发项", 编号 - 1)
    return 格式化大类触发项列表(编号 - 1)


def 格式化大类触发项列表(大类序号: int) -> str:
    大类 = 帮助大类[大类序号]
    行列表 = [f"{大类['名称']}：", ""]
    当前小类序号 = None
    for 全局序号, (小类序号, _, 触发项) in enumerate(枚举大类触发项(大类序号), start=1):
        if 小类序号 != 当前小类序号:
            if 当前小类序号 is not None:
                行列表.append("")
            行列表.append(大类["小类"][小类序号]["名称"])
            当前小类序号 = 小类序号
        行列表.append(f"{全局序号}. {触发项['触发']}")
    行列表.append("")
    行列表.append("0. 返回上一步")
    return "\n".join(行列表)


def 格式化帮助详情(大类序号: int, 触发编号文本: str) -> str:
    触发项列表 = 枚举大类触发项(大类序号)
...
def 处理帮助数字选择(会话键: str, 帮助状态: dict[str, Any], 编号文本: str) -> str:
    ...
    if 编号文本 == "0":
        return 返回帮助上一层(会话键, 层级, 大类序号)

    if 层级 == "大类":
        return 进入大类触发项(会话键, 编号文本)

    if 层级 in {"触发项", "详情"} and isinstance(大类序号, int):
        触发项序号 = int(编号文本) - 1 if 编号文本.isdigit() else None
        设置帮助状态(会话键, "详情", 大类序号, 触发项序号)
        return 格式化帮助详情(大类序号, 编号文本)

    设置帮助状态(会话键, "大类")
    return 格式化帮助大类()


def 返回帮助上一层(会话键: str, 层级: str, 大类序号: Any) -> str:
    if 层级 == "详情" and isinstance(大类序号, int):
        设置帮助状态(会话键, "触发项", 大类序号)
        return 格式化大类触发项列表(大类序号)
...
def 设置帮助状态(会话键: str, 层级: str, 大类序号: int | None = None, 触发项序号: int | None = None) -> None:
    待选择帮助会话[会话键] = {
        "时间": time.time(),
        "层级": 层级,
        "大类序号": 大类序号,
...
def 枚举大类触发项(大类序号: int) -> list[tuple[int, int, dict[str, Any]]]:
    """按全局顺序返回 [(小类序号, 小类内触发项序号, 触发项), ...]"""
    大类 = 帮助大类[大类序号]
    结果: list[tuple[int, int, dict[str, Any]]] = []
    for 小类序号, 小类 in enumerate(大类["小类"]):
        for 触发项序号, 触发项 in enumerate(小类["触发项"]):
            结果.append((小类序号, 触发项序号, 触发项))
    return 结果
...
def 格式化帮助详情MD(大类序号: int, 触发编号文本: str) -> str:
    触发项列表 = 枚举大类触发项(大类序号)
    if not 触发编号文本.isdigit():
        return f"帮助编号无效，请发送 1-{len(触发项列表)} 或 0 返回上一步"
    触发编号 = int(触发编号文本)
...
def 进入大类触发项MD(会话键: str, 编号文本: str) -> str:
    编号 = int(编号文本)
    if 编号 < 1 or 编号 > len(帮助大类):
        return f"帮助编号无效，请发送 1-{len(帮助大类)}"
    设置帮助状态(会话键, "触发项", 编号 - 1)
    return 格式化大类触发项列表MD(编号 - 1)
...
def 处理帮助数字选择MD(会话键: str, 帮助状态: dict[str, Any], 编号文本: str) -> str:
    ...
    if 编号文本 == "0":
        return 返回帮助上一层MD(会话键, 层级, 大类序号)

    if 层级 == "大类":
        return 进入大类触发项MD(会话键, 编号文本)

    if 层级 in {"触发项", "详情"} and isinstance(大类序号, int):
        触发项序号 = int(编号文本) - 1 if 编号文本.isdigit() else None
        设置帮助状态(会话键, "详情", 大类序号, 触发项序号)
        return 格式化帮助详情MD(大类序号, 编号文本)

    设置帮助状态(会话键, "大类")
    return 格式化帮助大类MD()


def 返回帮助上一层MD(会话键: str, 层级: str, 大类序号: Any) -> str:
    if 层级 == "详情" and isinstance(大类序号, int):
        设置帮助状态(会话键, "触发项", 大类序号)
        return 格式化大类触发项列表MD(大类序号)
    if 层级 == "触发项" and isinstance(大类序号, int):
        设置帮助状态(会话键, "大类")
        return 格式化帮助大类MD()
    待选择帮助会话.pop(会话键, None)
...
def 获取帮助键盘(会话键: str, 自动发送: bool = False) -> dict[str, Any] | None:
    ...
    if 层级 == "详情":
        ...
        触发项 = None
        if isinstance(大类序号, int) and isinstance(触发项序号, int):
            触发项列表 = 枚举大类触发项(大类序号)
            if 0 <= 触发项序号 < len(触发项列表):
                触发项 = 触发项列表[触发项序号][2]
        return 生成帮助详情键盘(触发项, 自动发送)
    return None


def 匹配任意层级名称(文本: str) -> tuple[str, int, int | None] | None:
    """跨层级匹配名称，返回 (目标层级, 大类序号(0-based), 触发项序号或None) 或 None；小类名不再匹配。"""
    for 序号, 大类 in enumerate(帮助大类):
        if 大类["名称"] == 文本:
            return ("触发项", 序号, None)
    for 大类序号, 大类 in enumerate(帮助大类):
        for 全局序号, (_, _, 触发项) in enumerate(枚举大类触发项(大类序号)):
            if 触发项["名称"] == 文本:
                return ("详情", 大类序号, 全局序号)
    return None


def 跳转到目标(会话键: str, 目标: tuple[str, int, int | None]) -> str:
    层级, 大类序号, 项目序号 = 目标
    if 层级 == "触发项":
        设置帮助状态(会话键, "触发项", 大类序号)
        return 格式化大类触发项列表(大类序号)
    if 层级 == "详情":
        设置帮助状态(会话键, "详情", 大类序号, 项目序号)
        return 格式化帮助详情(大类序号, str(项目序号 + 1))
    设置帮助状态(会话键, "大类")
    return 格式化帮助大类()


def 跳转到目标MD(会话键: str, 目标: tuple[str, int, int | None]) -> str:
    层级, 大类序号, 项目序号 = 目标
    if 层级 == "触发项":
        设置帮助状态(会话键, "触发项", 大类序号)
        return 格式化大类触发项列表MD(大类序号)
    if 层级 == "详情":
        设置帮助状态(会话键, "详情", 大类序号, 项目序号)
        return 格式化帮助详情MD(大类序号, str(项目序号 + 1))
    设置帮助状态(会话键, "大类")
    return 格式化帮助大类MD()
