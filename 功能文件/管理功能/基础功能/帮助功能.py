        ...
        "小类": [
            ...
            },
            {
                ...
                "触发项": [
                    ...
                    {"名称": "群文件清理", "触发": "登录群文件 / 群文件登录cookie / 清理群文件 / 群文件清理 / 清理全部群文件 / 添加群聊 群号 / 删除群聊 群号 / 删除群聊 / 查看群聊 / 更改备注 / 更改备注 群号", "详情": "通过 QQ 群文件网页接口删除群文件，需先「登录群文件」拿 Cookie；清理群文件列出已添加群列表后点击群号按钮清理对应群（120秒有效，点返回上一步退出）；清理全部群文件清「添加群聊」添加的全部群；点「删除群聊」按钮进入删除模式，点群号按钮直接删除，点「返回上一步」退出；点「更改备注」选群后 60 秒内下条消息作为该群备注，超时自动取消；查看群聊仅列出已添加群列表；网页接口只删根目录文件，不递归子文件夹。", "快捷命令": ["登录群文件", "清理群文件", "清理全部群文件", "查看群聊", "删除群聊", "更改备注"]},
