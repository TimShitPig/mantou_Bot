        ...
        "小类": [
            ...
            },
            {
                ...
                "触发项": [
                    ...
                    {"名称": "生成卡密", "按钮data": "卡密生成", "触发": "生成卡密 [数量]", "详情": "生成 12 位一次性激活卡密，默认 1 个。", "快捷命令": ["生成卡密 1", "生成卡密 5", "生成卡密 10", "生成卡密 20", "生成卡密 50", "生成卡密 100"]},
                    {"名称": "查询卡密", "按钮data": "卡密查询", "触发": "查询卡密 -> 1/2/0 / 查询卡密 [使用/没使用/筛选] / 下一页 / 下 / 上一页 / 上 / 复制 [天数]", "详情": "查询卡密先选择已使用或未使用；已有查询结果时，发送 复制 输出全部卡密，发送 复制 30 只输出 30 天卡密。", "快捷命令": ["下一页", "上一页"]},
                ],
            },
        ],
    },
...
def 生成小类触发项键盘(大类序号: int, 小类序号: int, 自动发送: bool = False) -> dict[str, Any]:
    ...
    for 序号, 触发项 in enumerate(小类["触发项"], start=1):
        ...
        if 用按钮字:
            ...
            按钮列表.append(生成按钮(data值, 触发项["名称"], 自动发送=自动发送))
        elif 触发项.get("快捷命令"):
