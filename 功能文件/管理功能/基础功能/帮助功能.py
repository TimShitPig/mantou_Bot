        ...
        "小类": [
            {
                "名称": "管理功能",
                "触发项": [
                    {"名称": "群禁言", "触发": "禁言 @成员 / 禁 @成员 1 / 解 @成员", "详情": "管理员可按 @成员 设置禁言时长或解除单个成员禁言；禁言默认 7 天，不带单位的数字按天计算，支持秒、分、时和天。单成员禁言成功后会提及被禁言成员并提示联系群主。", "快捷命令": ["禁言 @成员", "禁 @成员 1", "解 @成员"]},
                    {"名称": "小说开关", "触发": "小说 / 小说列表 / 开小说 / 关小说 / 开番茄 / 开七猫 / 开书旗 / 开QQ阅读 / 开QQ浏览器 / 开得间 / 开点众 / 开知乎 / 开塔读 / 开百度 / 开小米 / 开宜搜 / 开米读 / 开猫眼 / 开酷我 / 开酷匠 / 开连城 / 开菠萝包 / 开晋江 / 关闭对应功能 / 开测试 / 关测试", "详情": "发送「小说」或「小说列表」可查看全局小说功能和各平台状态。管理员发送「关小说」会关闭全部群聊的小说下载与找书；发送「开小说」只恢复全局开关，已单独关闭的平台继续关闭。管理员可用「开平台名」或「开启平台名」切换各平台下载功能。", "快捷命令": ["小说", "开小说", "关小说", "开番茄", "开七猫", "开书旗", "开QQ阅读", "开QQ浏览器", "开得间", "开点众", "开知乎", "开塔读", "开百度", "开小米", "开宜搜", "开米读", "开猫眼", "开酷我", "开酷匠", "开连城", "开菠萝包", "开晋江", "开测试", "关测试"]},
        ...
        "小类": [
            {
                "名称": "小说功能",
                "触发项": [
                    {"名称": "找书", "触发": "找关键词 / 找书 关键词 / 找书名 关键词 / 找作者 关键词 / 上一页 / 下一页", "详情": "聚合搜索番茄、七猫、QQ阅读、QQ浏览器、书旗、得间、点众、塔读、百度小说、小米小说、宜搜、米读、猫眼、酷我、酷匠、连城、菠萝包和晋江；可自动识别或显式指定书名、作者，按匹配方式、实际匹配字数、覆盖率和跨平台共识排序，无关结果不展示；每页 5 条，不显示链接和来源。"},
                    {"名称": "七猫小说", "触发": "七猫链接/七猫分享卡片", "详情": "识别七猫链接或分享卡片，下载完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "书旗小说", "触发": "书旗链接/书旗分享卡片", "详情": "识别书旗长篇、短篇和短链；一次读取完整目录后按正文 URL 动态并发下载单章，仅重试失败章节，缺章时停止；完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    ...
                    {"名称": "QQ浏览器小说", "触发": "QQ浏览器小说链接 / qb://ext/novelreader 深链", "详情": "使用 QQ 浏览器公开搜索、详情、目录和正文接口，按番茄格式生成完整 TXT；不包含 Cookie、EPUB、封面或独立上传功能。"},
                    ...
                    {"名称": "知乎", "触发": "知乎 story 分享链接/分享卡片", "详情": "识别 story.zhihu.com 的 paid_column 分享链接，先请求目录接口获取完整章节，再用章节详情补充书籍信息并发提取正文；失败章节单独重试，缺章不生成文件，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "塔读小说", "触发": "塔读链接/找书结果", "详情": "识别 reader.tadu.com 书籍链接和找书结果，使用塔读 App 接口获取详情、目录和 TDZ 正文；目录和正文通过异步 HTTP 连接复用，最高 400 路并发，内嵌加密与正文解析逻辑，缺章不生成部分 TXT，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "百度小说", "触发": "百度小说链接/百度小说分享卡片", "详情": "识别百度小说链接或分享卡片，使用百度详情、目录和 AES 正文接口，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "小米小说", "触发": "小米小说链接/小米小说分享卡片", "详情": "识别小米浏览器小说链接或分享卡片，使用详情、目录和正文接口，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "宜搜小说", "触发": "宜搜小说链接/分享卡片", "详情": "识别宜搜链接或分享卡片，使用异步复用会话和库解密正文，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "米读小说", "触发": "米读小说链接/分享卡片", "详情": "识别米读链接或分享卡片，使用异步复用会话读取详情、目录和正文，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "猫眼小说", "触发": "猫眼/掌阅链接", "详情": "识别猫眼或掌阅链接，使用异步复用会话读取详情、目录和正文，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "酷我小说", "触发": "酷我小说链接/分享卡片", "详情": "识别酷我链接，使用异步复用会话读取详情、目录和明文正文，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "酷匠小说", "触发": "酷匠小说链接/分享卡片", "详情": "识别酷匠链接，使用异步复用会话和库解密正文，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "连城小说", "触发": "连城小说链接/分享卡片", "详情": "识别连城链接，使用异步复用会话读取详情、目录和 HTML 正文，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "菠萝包小说", "触发": "菠萝包小说链接/分享卡片", "详情": "识别菠萝包链接，使用异步复用会话读取详情、目录和明文正文，按番茄格式生成完整 TXT；缺少任意章节时停止，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                    {"名称": "晋江小说", "触发": "晋江链接/分享卡片", "详情": "识别晋江书籍链接或分享卡片，只下载接口确认可公开读取的章节；复用异步 HTTP 会话并发请求明文正文，付费章节不请求，缺少任意公开章节时不生成部分 TXT，完成后上传当前选择的小说网盘并发送“点击打开”按钮。"},
                ],
            },
            {
                "名称": "群管功能",
                "触发项": [
                    {"名称": "成员禁言", "触发": "禁言 @成员 / 禁 @成员 1 / 解除禁言 @成员 / 解 @成员", "详情": "管理员可按 @成员 设置禁言时长；禁言默认 7 天，不带单位的数字按天计算，支持秒、分、时和天，成功后会 @被禁言成员并提示联系群主，使用解除禁言或解立即解除。"},
                    {"名称": "数字撤回", "触发": "连续 9-12 位数字", "详情": "QQ群主和管理员不撤回；普通成员撤回当前消息并拉取 100 条群历史，最多撤回该用户最近 8 条消息。"},
                    {"名称": "卡片撤回", "触发": "群名片/JSON 卡片/群分享卡片", "详情": "QQ群主和管理员不撤回；普通成员自动撤回当前卡片消息，并最多撤回该用户最近 8 条消息；成功禁言后只发送一条发送者提醒。"},
                    {"名称": "合并转发撤回", "触发": "合并转发/聊天记录", "详情": "QQ群主和管理员不撤回；普通成员自动撤回当前合并转发消息，并最多撤回该用户最近 8 条消息；成功禁言后只发送一条发送者提醒。"},
                    {"名称": "QQ闪传撤回", "触发": "QQ闪传", "详情": "QQ群主和管理员不撤回；普通成员自动撤回当前 QQ 闪传消息，并最多撤回该用户最近 8 条消息；成功禁言后只发送一条发送者提醒。"},
                ],
            },
        ],
    },
