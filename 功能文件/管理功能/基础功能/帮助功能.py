        ...
        "小类": [
            {
                ...
                "触发项": [
                    ...
                    {"名称": "QQ阅读登录", "触发": "直接发送含 ywguid / ywkey 的 Cookie", "详情": "直接粘贴 QQ阅读 Cookie，无需命令前缀；支持普通 Cookie、Cookie 头、curl、Cookie Editor JSON 和 cookies.txt。有效登录态保存到 MySQL，新 Cookie 会覆盖旧登录态。"},
        ...
        "小类": [
            {
                ...
                "触发项": [
                    ...
                    {"名称": "QQ阅读", "触发": "QQ阅读详情/目录/分享链接", "详情": "识别 QQ阅读链接，使用 App 身份、csigs 签名、动态密钥池和 Fetcher 下载正文；数据库存在 QQ阅读登录态时以最新 Cookie 的 ywguid/ywkey 覆盖固定账号字段；site=4 出版书同时解析 CTEB 与 EPUB 加密资源；完整合成 TXT 后上传 UC 网盘并发送“点击打开”按钮。"},
