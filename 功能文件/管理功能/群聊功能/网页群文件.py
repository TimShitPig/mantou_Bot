...
- 更改备注 群号：进入备注等待（60 秒），下条消息作为该群备注保存，超时自动取消
"""
...
async def 处理查看群聊返回(event: Any, 文本: str, 配置: Any) -> str | None:
    """查看群聊状态下收到「返回上一步」：退出查看群聊状态，跳转回「群文件清理」触发项详情页。

    固定回到群文件清理详情页，避免帮助状态过期导致越级回大类。
    QQ 官方机器人发送 markdown + 键盘；普通机器人返回纯文本提示。
    """
...
async def 显示群文件清理详情页(event: Any, 配置: Any) -> str:
    ...
    if 目标 is None:
        ...
        md = 跳转到目标MD(会话键, 目标)
    群号 = 获取群号(event)
    键盘 = 获取帮助键盘(会话键, 自动发送=True, 配置=配置, 群号=群号)
    if 是QQ官方机器人(event):
