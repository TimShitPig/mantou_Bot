...
def 获取欢迎回调内容(文本: str) -> tuple[str, dict[str, Any]] | None:
    标识 = 解析欢迎回调命令(文本)
    if 标识 is None:
        return None
    ...
    内容 = {
        ...
        "小说列表": "\n".join([
            "## 可下载小说\n",
            "目前支持：**番茄小说、七猫小说、书旗小说、QQ阅读、得间小说、点众小说**。",
            "直接发送上述平台的书籍分享链接或分享卡片即可下载；也可以用找书功能从多个平台搜索。",
            "搜索结果会优先保留匹配度高且可下载的书籍，章节单独付费的 QQ 阅读书籍不会显示。",
        ]),
        "如何下载": "\n".join([
            "## 如何下载小说\n",
