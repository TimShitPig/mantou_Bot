...
![version](https://img.shields.io/badge/version-v5.61.2-2ea44f)
...
| QQ阅读 | 详情/目录/分享链接、Cookie | 动态网关签名、动态密钥池、libfock 解密；免费书下载全量，VIP 书按账号权限下载，单章付费书只下载免费章节 |
...
所有小说模块都复用统一的 TXT 合成和网盘出口。正文写入 `功能文件/下载缓存/`，仅用于所有已启用网盘生成分享链接，不直接通过 QQ 发送 txt 文件。插件启动或重载时会清理上次运行遗留的非占用缓存；运行期间每天按服务器本地时间零点清理文件日期早于当天的小说 TXT，当天文件和仍在上传或等待续传的文件会保留。

至少一个已启用网盘上传成功后，QQ 官方机器人会按平台发送对应的链接按钮；只有一个网盘成功时按钮显示“点击我打开”，多个网盘成功时按平台显示且每行最多两个；所有已启用网盘都未配置或上传失败时只回复 `文件发送失败，请稍后再试`。
...
- QQ阅读：动态网关签名、动态密钥池和 libfock 解密；免费书下载全量，非 VIP 账号对 VIP 书只请求免费章节，VIP 账号请求 VIP 书完整正文，单章付费书始终只请求免费章节；普通正文每批 200 章、最多 100 路并发；支持 Cookie 登录态写入数据库。
