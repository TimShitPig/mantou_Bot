...
| 管理功能 | 小说网盘 | `网盘` / `网盘状态` / `当前网盘` / `换UC` / `换夸克` / `换百度` / 直接粘贴三种网盘 Cookie | 仅群文件清理管理员白名单 QQ 可用；查看或切换主分享网盘；直接粘贴 UC、夸克或百度 Cookie 时自动识别并覆盖 MySQL 登录态，支持请求头、curl、Cookie Editor JSON 和 cookies.txt |
...
群文件清理管理员白名单内的 QQ 可发送 `网盘` / `网盘状态` / `当前网盘` 查看 UC、夸克和百度网盘配置状态；发送 `换UC`、`换夸克` 或 `换百度` 切换所有小说下载共用的主分享网盘。选择结果写入 MySQL `mantou_runtime_state`（namespace=`novel_share_pan`，state_key=`active`），重载后继续生效；只调用当前选择的网盘，不自动回退其他网盘。管理员还可直接粘贴普通 Cookie、`Cookie:` 请求头、curl、Cookie Editor JSON 或 Netscape cookies.txt；插件按字段自动识别平台，识别不明确时可加 `UC Cookie:`、`夸克 Cookie:` 或 `百度 Cookie:` 前缀。
...
夸克 Cookie 可继续填写到 `quark_pan_cookie`，也可由管理员直接粘贴包含 `b-user-id` / `__uid` 和 `__puus` 的完整 Cookie；聊天保存值写入 `novel_pan_auth/quark` 并优先于配置。Cookie 没有 `ctoken` 时会先访问夸克网盘主页尝试补全。上传目录读取 `quark_pan_upload_dir`，默认 `/小说机器人`。插件自动逐级创建目录、删除同名旧文件、完成预上传与 OSS 单分片上传，并通过夸克分享接口取得短链接；数据库登录态使用期间刷新 `__puus` 或 `ctoken` 后会受控回写最新值。
