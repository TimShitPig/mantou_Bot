...
![version](https://img.shields.io/badge/version-v5.72.0-2ea44f)
...
| QQ阅读 | 详情/目录/分享链接、Cookie | 自动判断账号 VIP；非 VIP 按原始目录整本请求第三方明文正文，VIP 账号走原账号正文接口，单章付费书沿用官方账号规则 |
...
- QQ阅读：自动读取详情返回的账号 VIP 状态；非 VIP 账号按原始目录整本使用第三方明文正文接口，VIP 账号请求原账号正文接口；VIP 账号的单章付费书沿用官方免费章节规则；账号正文每批 200 章、最多 100 路并发，第三方正文按最多 200 章分段、最多 16 路并发；支持 Cookie 登录态写入数据库。
