...
夸克 Cookie 可继续填写到 `quark_pan_cookie`，也可由管理员私聊发送 `夸克登录` 扫码自动获取，或直接粘贴包含 `b-user-id` / `__uid` 和 `__puus` 的完整 Cookie；聊天保存值写入 `novel_pan_auth/quark` 并优先于配置。夸克请求完全按当前网页端 `drive-pc.quark.cn` 参数执行，短时 `ctoken` 不再加入请求参数；上传目录读取 `quark_pan_upload_dir`，默认 `/小说机器人`。插件逐级创建目录、删除同名旧文件并等待删除任务完成，再按网页端预上传、哈希校验、OSS 单分片和完成上传签名创建文件，最后通过分享接口取得短链接；请求期间刷新 `__puus` 时会受控回写最新值。
