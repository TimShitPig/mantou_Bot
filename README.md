...
| 管理功能 | 用户激活 | `@用户 激活` / `生成卡密` / `查询卡密` / `查询时间` / `查询用户` | 管理员可 @ 或填写 QQ 号激活、重置、增减时间、生成卡密并查询用户 |
...
群文件清理管理员白名单内的 QQ 可发送 `@用户 激活` 或 `激活 @用户` 激活成员，未填写天数时默认 30 天；也可发送 `@用户 激活 90` 或 `激活 用户QQ 90` 激活 90 天。已激活用户再次使用普通激活时不会重复写入，会提示用户已激活。发送 `@用户 重置`、`重置 @用户` 或 `重置 用户QQ` 会取消该用户激活；管理员可以一次 @ 多个用户或填写多个 QQ 号批量激活、重置或查询时间。`@用户 增加 5` / `增加 用户QQ 5` 会在原有到期时间基础上增加 5 天，`@用户 减少 3` / `减少 用户QQ 3` 会减少 3 天，调整后已到期则取消激活。管理员发送 `生成卡密` 默认生成 1 个 12 位卡密，也可发送 `生成卡密 数量` 批量生成，单次最多 200 个；卡密默认激活 30 天，用户直接发送卡密给机器人后激活本人，同一卡密只能使用一次。管理员发送 `查询卡密` 可查看当前群卡密，每页 5 条；`查询卡密 已使用` / `查询卡密 使用的卡密` 查看已使用卡密，`查询卡密 未使用` / `查询卡密 没使用的卡密` 查看未使用卡密，也可用 `查询卡密 QQ号`、`查询卡密 @用户` 或 `查询卡密 卡密` 筛选，发送 `下一页` 翻页。管理员发送 `查询用户` 可查看当前群已激活用户，每页 10 条，发送 `下一页` 翻页。如果需要唤醒机器人后手写目标，可使用 `@机器人 激活 用户QQ`、`@机器人 重置 用户QQ` 或 `@机器人 查询时间 用户QQ`。普通用户发送 `查询时间` 可查询自己的激活状态、激活时间、到期时间和剩余时间；管理员可发送 `@用户 查询时间`、`查询时间 @用户`、`查询时间 用户QQ` 或 `@机器人 查询时间 用户QQ` 查询其他用户。激活、重置和查询命令都优先处理消息里被 @ 的用户，不会把 `@用户 激活` / `@用户 重置` / `@用户 查询时间` 识别成操作发送者本人；同时 @ 机器人和多个成员时会跳过机器人，只处理成员。激活记录按群和用户保存。未激活用户使用随机英文单词、随机一言、疯狂星期四、古诗词名句、七猫小说下载或番茄小说下载时，插件会回复 `请查看群公告查看激活方法`。管理员不受激活门禁限制。用户激活和卡密只使用 MySQL 存储，不再使用本地 JSON；插件会自动创建内置表 `mantou_user_activation` 和 `mantou_user_activation_cards`。
