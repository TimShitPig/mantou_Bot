...
![version](https://img.shields.io/badge/version-v6.1.81-2ea44f)
...
所有模块统一写入文件缓存分类。AstrBot 环境下写入数据目录的 `mantou_bot_file_cache/`，非标准本地目录使用 `功能文件/文件缓存/`；目录按 `小说缓存/`、`媒体缓存/`、`临时缓存/`、`消息记录缓存/` 和 `其他缓存/` 分类。插件启动时会把旧小说缓存、媒体副本、临时上传文件及旧消息缓存迁移到新目录；上传中的小说文件和续传任务保留原路径直到可安全迁移。小说 TXT 只用于所有已启用网盘生成分享链接，不直接通过 QQ 发送；插件启动或重载时会清理上次运行遗留的非占用缓存，运行期间每天按服务器本地时间零点清理本地小说缓存和 UC、夸克、百度上传目录中前两天及更早的 TXT，今天、昨天以及仍在上传或等待续传的文件会保留。

聊天收到的图片、视频和普通文件只保存 QQ 官方附件地址及消息数据，不会下载到服务器；网页读取时按需代理且不落盘。网页发送的字节媒体由浏览器 Cache API 或 IndexedDB 缓存，服务器不新增媒体副本；旧版本留下的媒体文件迁入 `媒体缓存/` 并继续按过期时间清理。语音转码和网页上传过程中的生成文件写入 `临时缓存/`。

网页或机器人发送图片、视频和文件时使用 QQ 官方富媒体接口；聊天媒体不经过图床或 COS。
