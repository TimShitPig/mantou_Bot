...
| 管理功能 | 数字撤回 | `连续 9-12 位数字` | 自动撤回符合规则的数字消息，同群同成员触发 3 次后自动踢出 |
...
用户发送的消息中包含连续 9 到 12 位数字时，插件会尝试撤回该消息。同一成员在同一个群里成功触发数字撤回 3 次后，会调用当前 OneBot 适配器的 `set_group_kick` 踢出该成员。

只统计数字规则触发；群名片、合并转发、QQ 闪传等其它撤回类型不计入这 3 次。
