...
![version](https://img.shields.io/badge/version-v6.1.10-2ea44f)
...
| 功能 | 触发文本 | 说明 |
| --- | --- | --- |
| 数字撤回 | QQ 官方群内连续 9-12 位数字 | 普通成员触发后只撤回当前消息；QQ 群主和管理员不撤回 |
| 跨群累计禁言 | QQ 官方群广告类消息撤回 | 同一用户在不同群触发时按跨群累计次数递增禁言时长，群2不会重新从 0 计算；只禁言当前触发的 QQ 官方群 |
| 卡片撤回 | QQ 官方群群名片 / 群分享 / JSON 卡片 / 空间相册分享 / 合并转发 / QQ 闪传 | 自动撤回对应消息 |
