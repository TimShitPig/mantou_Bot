...
群文件清理只调用当前 AstrBot 适配器暴露的 OneBot 扩展接口 `get_group_root_files`、`get_group_files_by_folder` 和 `delete_group_file`，不会连接 NapCat 或 NapCat WebUI。`qq_official` 事件只返回 `group_openid`，不能直接当作数字 QQ 群号；当前适配器没有 `api.call_action` 或返回非数字群号时，插件会输出 `群文件清理事件诊断` 日志用于查看真实事件参数。
