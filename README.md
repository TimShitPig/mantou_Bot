...
番茄小说识别 `fanqienovel.com`、`changdunovel.com`、`fqnovel.com` 和 `novelfm.com` 链接，支持 JSON 卡片中的链接。下载前会先回复书名、作者、状态、章节、字数、简介和 `正在下载中请稍等.....`，正文按约 500 万字分段请求后合并为一个 txt。文件发送与七猫一致：写入 `功能文件/下载缓存/`，先尝试裸本地路径，再尝试 `file://` URI，上传尝试结束后删除缓存 txt，成功时不额外发送完成提示。
