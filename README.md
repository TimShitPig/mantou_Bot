...
| 管理功能 | QQ阅读 | QQ阅读详情/目录链接、`book.qq.com/book-detail/数字`、`novel.html5.qq.com/portal/novel-intro?bookid=数字`、`QQ阅读登录`、直接发送 QQ阅读 Cookie、`QQ阅读cookie状态` | 使用 QQ阅读本地 App 批量下载，完成后上传 UC 并发送“点击打开”按钮；管理员可用手机号登录或直接发送浏览器 Cookie、curl、Cookie Editor JSON、cookies.txt 保存登录态；新登录成功后覆盖旧登录态，失败时保留旧登录态 |
