...
![version](https://img.shields.io/badge/version-v6.1.46-2ea44f)
...
| QQ阅读 | 详情/目录/分享链接、Cookie | 自动读取账号 VIP 状态；当前正文统一优先使用官方账号接口，单章付费书仍只请求免费章节 |
...
- QQ阅读：读取账号级 VIP 状态并记录鉴权结果；当前正文统一优先使用官方账号正文接口，单章付费书仍只请求免费章节；账号正文每批 200 章、最多 100 路并发；支持 Cookie 登录态写入数据库。
