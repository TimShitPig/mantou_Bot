...
| 管理功能 | 群管功能 | `连续 9-12 位数字` / 群名片 / 合并转发 / QQ 闪传 / `@用户 踢` | 数字撤回成功 3 次后自动踢出；管理员可直接 @ 用户踢出 |
...
| 管理功能 | 用户激活 | `@用户 激活` / `@用户 激活 天数` / `@用户 重置` / `查询时间` | 用户可查询自己的激活时间、到期时间，管理员可 @ 用户查询或重置 |
...
群文件清理管理员白名单内的 QQ 可发送 `@用户 踢` 或 `@用户 踢了` 直接踢出成员。
...
| 配置项 | 说明 |
| --- | --- |
| `group_file_cleanup_admin_qq` | 群文件清理管理员 QQ 白名单，可使用群文件清理、授权链接、番茄小说API切换、用户激活、用户重置、查询其他用户激活时间和 @用户踢人 |
...
群文件清理管理员白名单内的 QQ 可发送 `@用户 激活` 或 `激活 @用户` 激活成员，未填写天数时默认 30 天；也可发送 `@用户 激活 90` 激活 90 天。发送 `@用户 重置` 或 `重置 @用户` 会取消该用户激活；如果需要唤醒机器人后手写目标，可使用 `@机器人 激活 用户QQ` 或 `@机器人 重置 用户QQ`。普通用户发送 `查询时间` 可查询自己的激活状态、激活时间、到期时间和剩余时间；管理员可发送 `@用户 查询时间`、`查询时间 @用户` 或 `@机器人 查询时间 用户QQ` 查询其他用户。激活、重置和查询命令都优先处理消息里被 @ 的用户，不会把 `@用户 激活` / `@用户 重置` / `@用户 查询时间` 识别成操作发送者本人。激活记录按群和用户保存。未激活用户使用随机英文单词、随机一言、疯狂星期四、古诗词名句、七猫小说下载或番茄小说下载时，插件会回复 `请查看群公告查看激活方法`。管理员不受激活门禁限制。数据库启用后插件会连接 MySQL 并自动创建 `group_id`、`user_id`、`expires_at`、`updated_at` 字段的激活记录表；数据库未启用时使用本地 JSON 文件持久化。
