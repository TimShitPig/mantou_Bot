...
![version](https://img.shields.io/badge/version-v6.1.48-2ea44f)
...
| QQ阅读 | 详情/目录/分享链接、Cookie | 恢复第三方接口接入前的官方账号下载版本，保留 Cookie 登录、官方目录及正文链路 |
...
- QQ阅读：模块恢复至第三方正文接口接入前的最后版本（提交 `20d1cc5`），移除后续第三方正文及账号 VIP 鉴权改动；使用该版本的官方目录、账号正文和可下载章节筛选逻辑，正文每批 200 章、最多 100 路并发，支持 Cookie 登录态写入数据库；其他功能保持当前版本。
