...
| API功能 / 析API | QQ阅读 | QQ阅读详情/目录链接、`book.qq.com/book-detail/数字`、`novel.html5.qq.com/portal/novel-intro?bookid=数字` 或 `154.12.91.167:7000/content` 链接 | 使用 QQ阅读详情、目录和析API正文接口下载，正文每批 30 章、10 个动态并发，完成后上传 UC 并发送“点击打开”按钮 |
...
插件配置里填写 `uc_pan_cookie` 后，番茄、七猫、书旗、QQ阅读及旧番茄 API 下载完成会先把完整原小说 txt 上传到 UC 网盘，上传目录读取 `uc_pan_upload_dir`，默认 `/小说机器人`。上传前会分页扫描并删除目标目录下与原小说文件名完全相同的远端普通文件，不删除同名文件夹；如果创建上传目录时返回同名冲突，会重新查找已有目录继续上传；如果 UC 上传仍返回同名冲突，插件会重新扫描并删除同名旧文件，再继续使用原小说文件名重试，不再改用备用远端文件名；如果上传完成接口返回 `23008` / `file is doloading`，插件会复用预上传阶段返回的文件 ID 继续创建分享，创建分享时文件仍在处理中也会等待重试。上传成功后 QQ 官方机器人发送“您的 / 书名 / 作者 / 已经下载完成”和“点击打开”链接按钮，分享链接不写入消息正文；不再生成链接 txt 或发送原小说 txt。UC 未配置或上传失败时仅回复 `文件发送失败，请稍后再试`。
...
番茄小说及旧番茄 API 写入 `功能文件/下载缓存/` 后仅用于 UC 上传和百度后台备份；不再通过 AstrBot `File` 组件或 OneBot 文件接口发送 txt。UC 上传成功后发送下载完成提示与“点击打开”按钮，源缓存由百度后台任务结束后清理；UC 未配置或上传失败时只回复固定的文件发送失败提示。
