...
![version](https://img.shields.io/badge/version-v5.55.5-2ea44f)
...
| QQ阅读 | 详情/目录/分享链接、Cookie | 动态网关签名、动态密钥池、libfock 解密；非会员下载免费章节，VIP 账号下载完整正文 |
...
- QQ阅读：动态网关签名、动态密钥池和 libfock 解密；非会员只请求免费章节，VIP 账号请求完整正文；普通正文每批 200 章、最多 100 路并发；支持 Cookie 登录态写入数据库。
