...
| 管理功能 | 小说网盘 | `网盘` / `网盘状态` / `当前网盘` / `换UC` / `换夸克` / `换百度` | 仅群文件清理管理员白名单 QQ 可用；查看三种网盘配置状态并切换所有小说使用的主分享网盘，结果写入 MySQL，默认 UC，不自动回退其他网盘 |
...
| 配置项 | 说明 |
| --- | --- |
| `group_file_cleanup_admin_qq` | 群文件清理管理员 QQ 白名单，可使用帮助、状态、群文件清理、全部群文件清理、群文件状态、全量消息、小说网盘切换、保存 QQ阅读 Cookie、小说开关和禁言指令 |
...
小说下载功能全部免费可用，不保留用户激活、卡密、收费、付费或每日免费额度。运行状态仅使用 MySQL 的 `mantou_runtime_state` 表保存群文件 Cookie、QQ阅读登录态、待清理群列表、小说功能开关和当前主网盘；数据库配置在插件配置最下面的 `database_settings` 分类内，数据库名默认与用户名相同。未配置数据库时不会尝试连接 MySQL，小说功能默认全部开启，主网盘默认 UC，相关运行状态无法持久化。群文件清理管理员白名单内的 QQ 可直接发送含 `ywguid` 和 `ywkey` 的 QQ阅读 Cookie，无需命令前缀；插件只保存这两个必要字段，新 Cookie 通过同一状态键覆盖旧值，下载时读取最新值覆盖模块固定 `uid/usid`。动态密钥池仍只保存在当前进程内，不写入本地 `keypool.cache`。
...
群文件清理管理员白名单内的 QQ 可发送 `网盘` / `网盘状态` / `当前网盘` 查看 UC、夸克和百度网盘配置状态；发送 `换UC`、`换夸克` 或 `换百度` 切换所有小说下载共用的主分享网盘。选择结果写入 MySQL `mantou_runtime_state`（namespace=`novel_share_pan`，state_key=`active`），重载后继续生效；只调用当前选择的网盘，不自动回退其他网盘。

插件配置里填写 `uc_pan_cookie` 后可使用 UC 主分享。上传目录读取 `uc_pan_upload_dir`，默认 `/小说机器人`。上传前会分页扫描并删除目标目录下与原小说文件名完全相同的远端普通文件，不删除同名文件夹；如果创建上传目录时返回同名冲突，会重新查找已有目录继续上传；如果 UC 上传仍返回同名冲突，插件会重新扫描并删除同名旧文件，再继续使用原小说文件名重试，不再改用备用远端文件名；如果上传完成接口返回 `23008` / `file is doloading`，插件会复用预上传阶段返回的文件 ID 继续创建分享，创建分享时文件仍在处理中也会等待重试。

插件配置里填写 `quark_pan_cookie` 后可使用夸克主分享，上传目录读取 `quark_pan_upload_dir`，默认 `/小说机器人`。插件自动逐级创建目录、删除同名旧文件、完成预上传与 OSS 单分片上传，并通过夸克分享接口取得短链接；响应刷新 `__puus` 时只更新当前进程内 Cookie。

插件配置里填写 `baidu_pan_cookie` 后可使用百度主分享；上传完成后创建带提取码的永久分享链接，提取码写入链接查询参数供按钮直接打开。选择 UC 或夸克为主网盘时，百度仍按 `baidu_pan_upload_status` 做后台备份；完结同名文件已存在时跳过，连载删除同名普通文件后重传。百度作为当前主分享网盘时跳过重复后台备份，且不受后台状态筛选限制。
