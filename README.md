...
当前版本已移除激活与收费门禁，小说下载功能全部免费可用。用户激活、卡密、每日免费额度、全局/群聊/私聊付费开关状态和小说功能开关状态只使用 MySQL 存储，不再使用本地 JSON；数据库配置在插件配置最下面的 `database_settings` 分类内，数据库名默认与用户名相同，插件会自动创建内置表 `mantou_user_activation`、`mantou_user_activation_cards`、`mantou_user_free_quota` 和 `mantou_runtime_state`。
...
群文件清理管理员白名单内的 QQ 可发送 `状态` 查看系统信息、运行内存、运行 CPU、系统内存、系统 CPU、系统运行时间、插件运行时间、插件版本、番茄/七猫/书旗/QQ阅读开关、全局收费、当前群收费、私聊收费、每日免费额度、数据库配置状态、UC/百度网盘启用状态和群 cookie 生效/失效状态。状态命令不会显示网盘 Cookie 原文。
