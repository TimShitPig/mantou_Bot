...
![version](https://img.shields.io/badge/version-v6.1.74-2ea44f)
...
将域名或反向代理转发到监听端口后重载插件。QQ 官方帮助菜单会显示“打开网页版帮助”按钮。消息收发、群管和文件发送只处理 QQ 官方适配器（`qq_official`）。网页使用响应式布局，手机和电脑均可访问。控制台和登录页右上角都可以选择浅色模式或深色模式，选择会保存在当前浏览器并带切换动画。服务器位于 NAT 或容器网络时，需要把监听端口映射到外网。
...
| 功能 | 触发文本 | 说明 |
| --- | --- | --- |
| 数字撤回 | QQ 官方群内连续 9-12 位数字 | 普通成员触发后撤回同群该用户最近最多 30 条（含触发消息）；QQ 群主和管理员不撤回 |
| 跨群累计禁言 | QQ 官方群广告类消息撤回 | 同一用户在不同群触发时按跨群累计次数递增禁言时长，群2不会重新从 0 计算；只禁言当前触发的 QQ 官方群 |
| 卡片撤回 | QQ 官方群群名片 / 群分享 / JSON 卡片 / 空间相册分享 / 合并转发 / QQ 闪传 | 命中后联动撤回同群该用户最近最多 30 条 |
| 广告拦截开关 | `开广告` / `关广告` / `广告状态` | 插件管理员可按 QQ 官方群独立控制自动撤回，默认关闭，状态保存到 MySQL |
| 单成员禁言 | `禁言 @成员` / `禁 @成员 1` / `解除禁言 @成员` / `解 @成员` | 默认禁言 7 天，支持秒、分、时、天 |
| 网页禁言状态 | 消息记录页群聊 | 按 QQ 官方禁言状态接口显示被禁言成员、灰色头像、实时剩余时间，并可解除禁言 |
| 网页成员头像 | 消息记录页群聊 | 官方事件/历史记录提供头像时使用原图；没有头像时使用腾讯官方 AppID + OpenID 地址 |

广告联动撤回仅使用机器人已经收到并保存在内存/MySQL 的消息记录，不按两分钟过滤，不拉取其他适配器历史；逐条调用 QQ 官方撤回接口，成功后同步网页撤回标记。超过两分钟的消息也会提交，实际权限和时限以官方接口返回为准（官方文档仍列有时限错误码 `40064004`）。同一批最多 3 路并发，网页和自动撤回共用低于 10 QPS 的请求调度。
...
| 得间小说 | 完整书籍链接 / 分享卡片 | 详情目录后立即提示；8 路地址分页与最高 128 路单章正文流水线；自定义 CTR 批量解密，缺章停止 |
...
QQ 官方群消息包含连续 9 到 12 位数字时，插件会尝试撤回。群名片、JSON 卡片、空间相册分享、合并转发和 QQ 闪传也会进入同一撤回模块。

QQ 群主和管理员不会被自动撤回；插件使用 QQ 官方事件中的成员角色，并在可用时查询官方群成员角色。

普通成员命中规则后，从已接收的本地消息记录联动撤回同群该用户最近最多 30 条（包含触发消息），不按消息年龄过滤。广告类消息撤回并成功禁言后，只会发送一条“@发送者 / 请勿发送此类消息 / 如果是小说请联系群主”提醒。
...
本模块不提供踢人和全体禁言功能。单成员禁言默认 7 天，`禁 @成员 1` 表示 1 天，带 `秒`、`分`、`时`、`天` 时按对应单位计算；消息收发和群管只使用 QQ 官方接口。
...
聊天中的 QQ 图片、视频和普通文件只记录官方附件地址，网页查看时按需代理，不在收到消息时下载或转存。网页或机器人发送的字节媒体使用 QQ 官方富媒体接口；网页发送的字节媒体由当前浏览器使用 Cache API 或 IndexedDB 保存，服务器不再写入新的媒体副本。图床和图片 COS 不参与聊天媒体流程。
...
- QQ阅读：使用官方账号会员状态校验、目录和正文链路；有效 VIP 登录态下载会员书完整目录，单章付费书仍只请求免费章节；正文每批 200 章、最多 100 路并发，支持 Cookie 登录态写入数据库。
- 得间：详情目录完成后立即提示下载；8 路地址分页与最高 128 路单章授权/正文下载重叠执行，复用异步 HTTP 会话并使用有界任务及地址队列。自定义 CTR 使用 NumPy 批量整数运算、PyCryptodome 异或和 `gmpy2`，保持单章密钥及原协议；独立解密进程最多 4 个且按可用 CPU 保留调度空间，空闲 60 秒回收，避免大量 Python 解密线程拖慢聊天。依赖包含 `numpy>=1.26,<3`、`loky>=3.5,<4`，缺少任意地址或正文不合成 TXT。
