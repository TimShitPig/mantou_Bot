...
- 用户激活功能放在 `功能文件/管理功能/用户激活.py`，`main.py` 只负责加载模块并调用 `处理用户激活`、`获取未激活拦截回复`。群文件清理管理员白名单 QQ 可用 `@用户 激活` 或 `@用户 激活 天数` 激活成员，未带天数默认 30 天；可用 `@用户 重置` 或 `重置 @用户` 取消该用户激活；普通用户可发送 `查询时间` 查询自己的激活状态、激活时间、到期时间和剩余时间；管理员可用 `@用户 查询时间`、`查询时间 @用户` 或 `@机器人 查询时间 用户QQ` 查询其他用户激活时间。激活、重置和查询时间支持一次 @ 多个用户批量操作，并且必须跳过机器人自己的 @。激活、重置和查询命令必须优先操作消息里被 @ 的用户，不得因为 `is_wake`、`is_atbot` 或 `is_at_or_wake_command` 把 `@用户 激活` / `@用户 重置` / `@用户 查询时间` 改成操作发送者本人；如果需要唤醒机器人并手写目标，可使用 `@机器人 激活 用户QQ`、`@机器人 重置 用户QQ` 或 `@机器人 查询时间 用户QQ`。@ 解析需兼容 `message`、`components`、`content` 组件字段、`message_obj.raw_message` 中的 `[CQ:at]` / `[At:QQ]`，以及可见文本里的 `@昵称 激活`、`@昵称(QQ) 激活`、`@昵称 重置`、`@昵称(QQ) 重置`、`@昵称 查询时间`、`@昵称(QQ) 查询时间`；激活命令触发时保留受控 `用户激活诊断` 日志。激活记录按群和用户保存。未激活用户使用随机英文单词、随机一言、疯狂星期四、古诗词名句、七猫小说下载和番茄小说下载时，只回复 `请查看群公告查看激活方法`。管理员不受激活门禁限制。
...
- 群管功能保留数字撤回逻辑：只撤回当前触发消息，不拉取历史消息，不撤回之前的消息。同一成员在同一个群里被数字撤回模块成功撤回 3 次后，调用 OneBot 标准 `set_group_kick` 踢出该成员；数字、群名片/JSON 卡片、合并转发和 QQ 闪传等本模块处理的撤回类型都计数。
...
- 群文件清理、授权链接、用户激活、`查看API`、`1` / `2` API 切换和 `@用户 踢` / `@用户 踢了` 只能由插件配置 `group_file_cleanup_admin_qq` 里的 QQ 使用；这是群文件清理管理员白名单，不等同于群管理员。`@用户 踢` / `@用户 踢了` 支持一次 @ 多个用户批量踢出，并且必须跳过机器人自己的 @。
...
- 番茄小说 API 通过指令手动切换：群文件清理管理员白名单 QQ 发送 `查看API` 列出站点，120 秒内发送 `1` 切换 OIAPI，发送 `2` 切换析API；当前选择写入 `功能文件/下载缓存/番茄小说API.json`。禁止 OIAPI 失败后自动切换析API，当前接口失败时必须直接返回当前接口错误。
