...
- QQ 官方机器人 `qq_official` 群聊发送普通 txt 文件容易在官方富媒体上传阶段返回 `call inner proxy error`；七猫和番茄在 QQ 官方群聊中应直接提示平台限制，不再构造 AstrBot `File` 组件触发重试。非 QQ 官方群聊仍可优先使用 AstrBot `File` 组件，无法使用时再回退 OneBot 的 `upload_group_file`/`upload_private_file`。
