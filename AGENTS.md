...
- 群文件清理、全部群文件清理、授权链接、帮助、用户激活、生成卡密、查询卡密、`查看API`、`1` / `2` API 切换、`@用户 踢` / `@用户 踢了`、`开启禁言` 和 `开启全部禁言` 只能由插件配置 `group_file_cleanup_admin_qq` 里的 QQ 使用；这是群文件清理管理员白名单，不等同于群管理员。`@用户 踢` / `@用户 踢了` 支持一次 @ 多个用户批量踢出，并且必须跳过机器人自己的 @。`开启禁言` 调用 OneBot `set_group_whole_ban enable=True` 开启当前群全员禁言；`开启全部禁言` 先调用 `get_group_list` 获取机器人所在数字 QQ 群，再逐群开启全员禁言。
