...
- 用户激活和卡密只使用 MySQL 存储，不再使用本地 JSON；连接信息来自 `_conf_schema.json` 里的 `user_activation_database_*` 配置。表名在代码内置固定为 `mantou_user_activation` 和 `mantou_user_activation_cards`，不要暴露成配置项。数据库未配置完整或依赖缺失时，未激活用户只能在 `user_activation_daily_free_quota` 仍有剩余额度时放行。每日免费额度使用次数写入 `功能文件/下载缓存/用户免费额度.json`，这是运行时状态，不是插件配置项。
