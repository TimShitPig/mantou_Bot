...
- 七猫小说只识别直接发送的七猫链接和七猫 JSON 分享卡片，不保留 `七猫搜索`、`七猫小说`、`七猫下载`、`七猫小说下载` 等文本命令。支持 `https://www.qimao.com/shuku/数字/`、`app-share.wtzw.com/.../article-detail/数字`、`app-share.wtzw.com/.../short-story-detail/数字` 或七猫 JSON 分享卡片，识别后下载 txt 发送到 QQ。下载前必须先回复书名、作者、状态、章节、字数和 `正在下载中请稍等.....`，不要发送封面图片。七猫小说文件名格式为 `[完结]书名：xxx 作者：xxx.txt` 或 `[连载]书名：xxx 作者：xxx.txt`，全部写入 `功能文件/下载缓存/` 后通过本地文件路径调用 `upload_group_file` 或 `upload_private_file` 上传，上传尝试结束必须删除缓存 txt，不要保存到仓库 `downloads`。
